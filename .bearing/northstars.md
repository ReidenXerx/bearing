...
- **NS-23** — **bearing ships JUDGMENT, not capability.** Every agent can already review code, spawn subagents, adopt a persona, read a doc. What none of them know is **WHEN** — a milestone is when to review with a panel, a 40-site trace is when to fan out, a compaction is when the goal gets lost. So a proposal that adds a capability the agent already has is not a feature; **the trigger is the feature**. Microscope is not "we can review", it is "review HERE, this way". Judge every module by whether it changes what the agent DOES at a moment it would otherwise have handled worse — and if you cannot state that moment in one sentence, there is nothing to ship.
- **NS-24** — **Delegated work gathers; the delegator concludes.** A subagent does MINIMAL OR ZERO REASONING: it returns what it saw — `file:line`, verbatim output, the exact query it ran — never a verdict. Reasoning is precisely where an agent holding less context than you drifts, so a subagent that concludes inserts a lossy summarizer between the evidence and the decision, and the fan-out becomes a drift AMPLIFIER — the opposite of this product. A report must also separate "looked and found nothing" from "never understood the task"; collapsing those is the same failure as reading a graph zero as absence.
