...
`npm run bearing:agent-brief` or READ `gitnexus://repo/__GITNEXUS_REPO__/context`. Stale or missing embeddings → **`npm run bearing:agent-refresh` first** (`required_permissions: ["all"]`). Hooks **block** Grep/Read/MCP/shell until refresh succeeds; classical tools only if refresh **fails** (say why). Never ask user to analyze.
...
**When the work is a WRITE, not a lookup** — the same decided change applied at many independent sites (logging coverage, an error envelope, the rest of a settled migration) — the shape is `bearing-invasive-minions` instead. Same four gates plus two, and both are hard failures rather than judgment calls: the transformation is **DECIDED** in full before you spawn, and the slices are **DISJOINT** — two subagents writing one file is a lost update, both report success, and nothing in either report shows it happened. **A gathering minion must not decide what is TRUE; an invasive one must not decide what to CHANGE** — the delegator still concludes, and the conclusion is the transformation. Each returns `WROTE file:line` (the exact text written), `VERIFIED` (the narrow check it ran on its own slice, not the full suite) and `SKIPPED` (a site whose shape your brief did not cover — that is a bug report about the brief). Spawn only from a clean tree or a dedicated branch, review the whole diff yourself, and decide BEFORE spawning whether a half-applied change is acceptable: a fan-out that half-fails leaves the codebase in a state that usually still compiles and passes, so it ships with two conventions and no record of which was intended.
