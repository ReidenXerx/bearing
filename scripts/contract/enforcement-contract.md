...
**When the work is a WRITE, not a lookup** — the same decided change applied at many independent sites (logging coverage, an error envelope, the rest of a settled migration) — the shape is `bearing-invasive-minions`. Same four gates plus two: the transformation is **DECIDED** in full before you spawn, and the slices are **DISJOINT** — two subagents writing one file is a lost update, both report success, and nothing in either report shows it. **A gathering minion must not decide what is TRUE; an invasive one must not decide what to CHANGE.** Each returns `WROTE file:line`, `VERIFIED` (a narrow check on its own slice, not the full suite) and `SKIPPED` — and SKIPPED is the point: three minions skipping the same shape means your brief had a hole, which a serial agent would instead have quietly decided nine times. **Write the brief even when you do not fan out.** It survives compaction, it keeps site 30 consistent with site 3, and a brief you cannot write is a change you have not finished thinking about.
