...
export async function pickRuntimeInteractive() {
  // Keys are POSITIONAL and computed HERE. Each adapter used to hardcode its own, with the retired
  // runtime owning the first slot, while `allKey` was `ADAPTERS.length + 1`. Removing that adapter
  // left three, so allKey collided with the last adapter's own key and `map[allKey] = 'all'`
  // overwrote it: picking that adapter installed everything, it was unreachable from the wizard,
  // and the first key answered nothing. A key computed in one place cannot disagree with itself.
  //
  // Claude Code leads, and the default is whatever leads. It is the only runtime that can enforce
  // (NS-14); when Cursor owned position 1 the default silently fell through to Zed, which offers
  // no gates — the wizard's most consequential answer, chosen by accident of list order.
  const ordered = [
    ...ADAPTERS.filter((a) => a.choice.value === 'claude'),
    ...ADAPTERS.filter((a) => a.choice.value !== 'claude'),
  ];
  const adapterChoices = ordered.map((a, i) => ({
    key: String(i + 1),
    label: a.choice.label,
    value: a.choice.value,
  }));
  const allKey = String(adapterChoices.length + 1);
  const choices = [
    ...adapterChoices.map(({ key, label }) => ({ key, label })),
    {
      key: allKey,
      // Derived from the registry, never spelled out: the hardcoded label went on promising Cursor
      // for a whole release after Cursor could no longer be installed (NS-20).
      label: `All — every adapter (${ordered.map((a) => a.choice.label.split(' — ')[0]).join(' + ')}) in the same repo`,
    },
  ];
  const map = Object.fromEntries(adapterChoices.map((ch) => [ch.key, ch.value]));
  map[allKey] = 'all';
...
export async function pickFeaturesInteractive() {
  ...
  FEATURES.forEach((f, i) => {
    const tags = [
      f.needsGitnexus ? `${c.yellow}[needs GitNexus MCP]${c.reset}` : '',
      // Say it where the choice is made. A module that writes a new top-level directory and adds a
      // dependency must be visibly opt-in, not quietly absent from the default (NS-20).
      f.recommended ? '' : `${c.yellow}[opt-in — writes .e2e/, needs Playwright]${c.reset}`,
    ].filter(Boolean).join(' ');
    console.log(`  ${c.cyan}${i + 1}${c.reset}  ${c.bold}${f.title}${c.reset}${tags ? ` ${tags}` : ''}`);
  ...
  try {
    for (;;) {
      const ans = (
        await rl.question(
          `${c.dim}Numbers (e.g. 1,2,3), "all", or Enter for the recommended set: ${c.reset}`,
        )
      ).trim();
      // Enter and "all" are DIFFERENT, and the prompt says so. Letting Enter mean "all" while an
      // opt-in module exists would install it for pressing return — and letting "all" quietly
      // exclude something would make the word a lie (NS-20).
      if (!ans) return FEATURES.filter((f) => f.recommended).map((f) => f.id);
      if (ans.toLowerCase() === 'all') return FEATURES.map((f) => f.id);
      const picked = ans
        .split(/[,\s]+/)
        .filter(Boolean)
