...
export function mergeAgentsMd(absTarget, repoName, features = null, persona, personaNote) {
...
export function removeZedSettings(absTarget) {
  ...
  // Drop the containers we created once they hold nothing of the user's. Without this, uninstall
  // wrote back `{"context_servers":{},"agent":{"profiles":{}},"language_models":{}}` — and since
  // install CREATES this file in a repo that never had a .zed/, the user was left with a settings
  // file, and a directory, for an editor they may not even use. AGENTS.md already unlinks when it
  // empties out; this is the same rule one adapter over.
  if (isEmptyObject(cfg.agent?.profiles)) delete cfg.agent?.profiles;
  for (const key of ["context_servers", "agent", "language_models"]) {
    if (isEmptyObject(cfg[key])) delete cfg[key];
  }
  // Only when nothing of theirs is left. A settings.json holding their theme, keymap or their own
  // context servers is their file and stays exactly as it is.
  if (!isEmptyObject(cfg)) {
    writeJson(settingsPath, cfg);
    return;
  }
  fs.unlinkSync(settingsPath);
  // rmdir is refused on a non-empty directory, so their other .zed/ files keep it alive.
  try {
...
/**
 * True ONLY for a plain `{}`. Anything else — a string, an array, a populated object — is content
 * we did not put there, so it is theirs and it stays. Deleting on "not an object" would read a
 * malformed settings file as an empty one and throw the user's file away.
 * @param {unknown} v
 */
...
export const zedAdapter = {
  ...
  wire(absTarget, { repoName, features, mcpTransport = STDIO_TRANSPORT, gitnexusCmd, persona, personaNote }) {
    ...
    mergeAgentsMd(absTarget, repoName, features, persona, personaNote);
  },
