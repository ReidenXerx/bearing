...
function writeCursorContract(absTarget, features, repoName, persona, personaNote) {
  ...
  const raw = substitutePlaceholders(fs.readFileSync(src, "utf8"), { repoName, persona, personaNote });
  const fm = raw.match(/^---\n[\s\S]*?\n---\n/);
...
export const cursorAdapter = {
  ...
  wire(absTarget, { features, mcpTransport = STDIO_TRANSPORT, gitnexusCmd, repoName, persona, personaNote } = {}) {
    ...
    const wrote = writeCursorContract(absTarget, features, repoName, persona, personaNote);
