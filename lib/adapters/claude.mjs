 ...
 * file, and Claude Code runs hooks with cwd at the project root. A relative path needs no shell at
 * all, so there is nothing left to expand differently. Reported and diagnosed in #18.
 */
const HOOK_CMD = (script) => `node .claude/hooks/${script}`;
