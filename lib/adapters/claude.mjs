...
/** The status line bearing ships, and the command Claude Code runs to render it. */
const STATUSLINE_SCRIPT = ".claude/statusline.sh";
// `bash <path>` rather than the bare path: chmodScripts() only marks scripts/,
// .cursor/hooks/ and .githooks/ executable, so this file arrives without a +x bit and
// executing it directly would fail with EACCES on every render.
// $CLAUDE_PROJECT_DIR is contractual for hooks and is in fact also set for status lines,
// but that half is undocumented — hence the cwd fallback rather than a bare expansion.
const STATUSLINE_CMD = `bash "\${CLAUDE_PROJECT_DIR:-.}/${STATUSLINE_SCRIPT}"`;

/** A status line is "ours" if it runs the script we install. */
function isOurStatusLine(sl) {
  return /\.claude\/statusline\.sh/.test(sl?.command ?? "");
}
...
function mergeClaudeSettings(absTarget, stealth = false) {
  ...
  // Status line. Same file-must-exist rule as the hooks above, so a filtered install
  // never points Claude at a script that did not ship. A status line the user configured
  // themselves is left exactly as it is: overwriting one is destructive AND invisible
  // until they next glance at the bottom of the screen, which is the worst way to find
  // out a tool rewrote your config. We only ever replace our own.
  if (fs.existsSync(path.join(absTarget, STATUSLINE_SCRIPT))) {
    if (!cfg.statusLine || isOurStatusLine(cfg.statusLine)) {
