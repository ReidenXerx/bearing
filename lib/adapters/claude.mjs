...
function mergeMcpJson(absTarget, transport = STDIO_TRANSPORT) {
  const mcpPath = path.join(absTarget, ".mcp.json");
  const cfg = readJsonSafe(mcpPath, { mcpServers: {} });
  cfg.mcpServers ??= {};
  cfg.mcpServers.gitnexus = mcpEntry(transport);
...
export const claudeAdapter = {
  ...
  wire(absTarget, { repoName, features, mcpTransport }) {
    ...
    if (!features || features.has("gitnexus")) mergeMcpJson(absTarget, mcpTransport);
