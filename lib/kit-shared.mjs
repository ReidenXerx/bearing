...
export function shouldCopyBundleFile(rel, runtime, features = null, targetHas = null) {
  ...
  // Seed hooks.json only when absent. Its own header says "Optional hook tuning, TEAM-SHARED
  // (committed)" and instructs users to edit it — re-copying on every update reverted their
  // settings (e.g. mode: guide -> enforce) without a word.
  if (rel === ".bearing/hooks.json") return !targetHas?.(rel);
