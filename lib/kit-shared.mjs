 ...
 *
 * `.e2e/core/session.js` is a CONTRACT STUB whose own docblock says "bearing ships the shape and
 * the scars; you write the body" — and until it joined this list, the next update deleted the body.
 * How an app holds a session is the most app-specific thing about it, so that file is hours of
 * someone's work on their auth: the storage keys, whether values are stored raw, which of
 * localStorage and sessionStorage the app's reader actually reads. Overwriting it with the throwing
 * stub does not merely lose the work — it silently returns the repo to the state the stub exists to
 * warn about, where a verifier runs logged-out and reports every feature as missing (NS-1).
 * Measured: implement the stub, run `bearing update`, and the implementation is gone.
 *
