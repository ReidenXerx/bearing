 ...
 */
// `.seeds/` is a per-run, per-machine ledger of fixtures a run created — pids, timestamps and
// the URLs that would delete them. It is state, not source, and belongs beside shots/ rather
// than in everyone's diff.
...
export function installStatusLine(opts = {}) {
  ...
  if (cfg.statusLine && !isOurs(cfg.statusLine)) {
    // Overwriting one is destructive AND invisible — you find out by noticing the bottom of your
    // screen changed. Print the line instead and let them decide.
    warn(
      `You already have a status line (${cfg.statusLine.command}) — left untouched.\n` +
        `    To switch: set "statusLine": { "type": "command", "command": "${command}" } in ${settingsPath}`,
    );
    return;
  }
