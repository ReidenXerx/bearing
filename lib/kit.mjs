...
function printInstallComplete(
  ...
  printNewModules(indexState.prevKitVersion, indexState.features);
}
...
function printNewModules(prevVersion, features) {
  ...
  // The delta form, so the line can be pasted. Naming the full replacement list here would be a
  // command that silently uninstalls whatever the reader forgets to retype.
...
export async function cliMain(argv, invokedAs = "node lib/kit.mjs") {
  ...
  if (!cmd || cmd === "--help" || cmd === "-h") {
    console.log(`Usage:
  ${invokedAs} install <target-repo> [--runtime cursor|zed|claude|both|all] [--features ${FEATURE_IDS.join(",")}|all] [--repo-name NAME] [--stealth] [--mcp http|stdio|PORT] [--gitnexus-cmd CMD] [--quick] [--no-setup] [--skip-verify] [--prettierignore|--no-prettierignore] [--test-order|--no-test-order]
  ...
  if (!resolvedRuntime && cmd === "install" && !interactive) {
    ...
    if (detected.length) {
      ...
      console.log(`    Not right? Re-run with --runtime cursor|zed|claude|codex|all`);
    } else if (process.stdin.isTTY) {
      resolvedRuntime = await pickRuntimeInteractive();
    } else {
      warn(
        `No agent environment detected and no TTY to ask — installing for "both" (cursor+zed).\n` +
