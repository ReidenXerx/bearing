...
import { migrateLegacyInstall, cleanupRetiredCursor } from "./migrate.mjs";
import {
  detectPrettier,
...
/** The seed-once file whose every word becomes the user's the moment they write in it. */
const NORTHSTARS_REL = ".bearing/northstars.md";

/**
 * Has anyone written an actual north-star into this file, or is it still the pristine starter?
 *
 * bearing seeds a starter so the module is not silently inert, and the starter deliberately
 * contains no `NS-<digit>` line — so "has a citable claim" is exactly "a human has been here".
 * That distinction decides whether the file may be deleted: an untouched starter is ours to take
 * back, and an authored one is the user's semantic anchor, unrecoverable if it was never
 * committed. NS-1 is named after this file being deleted once already.
 *
 * The pattern is deliberately the DIGEST's pattern (session-primer.mjs, northStarsDigest) — the
 * two must agree about what counts as a north-star, or a file the anchor happily reads could be
 * one this check calls empty and removes. A test asserts they agree.
 * @param {string} abs
 */
function hasAuthoredNorthStars(abs) {
  try {
    return fs
      .readFileSync(abs, "utf8")
      .split("\n")
      .some((l) => /^\s*(?:#{1,6}\s*)?(?:[-*+]\s*)?\**\s*NS-\d+\b/.test(l));
  } catch {
    return false;
  }
}
...
/**
 * Does this gold-practices file carry the PROJECT's own half? Anything below the END marker is
 * theirs — `PP-#` rules bearing never writes and must never take away.
 * @param {string} abs
 */
function hasProjectPractices(abs) {
  try {
    const text = fs.readFileSync(abs, "utf8");
    const i = text.indexOf("<!-- END GENERATED: gold-practices -->");
    return i >= 0 && /\*\*PP-\d+\*\*/.test(text.slice(i));
  } catch {
    return false;
  }
}
 ...
 */
// `.seeds/` is a per-run, per-machine ledger of what a run created — pids, timestamps and the
// URLs that would delete them. State, not source, and it belongs beside shots/ rather than in
// everyone's diff.
const GITIGNORE_E2E = [".e2e/node_modules/", ".e2e/shots/", ".e2e/.seeds/", ".e2e/storage*.json"];
...
export function installKit(targetRoot, opts = {}) {
  ...
  //
  // An explicit --features is a choice made NOW, with every current module id available, and is
  // honoured exactly as written. A set read back from the manifest was recorded before some of
  // today's modules existed, so it inherits whatever has since been split out of what it selected —
  // otherwise agreeing to a module in an older version is what deletes its file in a newer one.
  const recordedFeatures = readManifest(absTarget)?.data.features;
  const features = opts.features
    ? parseFeatures(opts.features)
    : recordedFeatures
      ? inheritSplitFeatures(parseFeatures(featureSpecOf(recordedFeatures)))
      : parseFeatures(defaultFeatureIds().join(","));
  // WHOSE `.e2e/` IS IT? Only a first install can tell, so the answer is recorded (NS-22 — the same
  // trap and the same fix as `createdGitignore`). Installing the harness over an existing one
  // replaces `core/report.js` with bearing's factory API, and every `verify/*.js` importing the old
  // names then dies with `check is not a function` on its next run — while the install exits 0 and
  // prints success (NS-1, NS-20). Backups mean nothing is LOST, but the project's e2e suite is
  // broken until someone notices. Five of the ten repos on this author's machine have their own
  // harness, and `--features all` — which the README and `--help` both advertise — selects e2e.
  const ownsE2e =
    readManifest(absTarget)?.data.ownsE2e ?? !fs.existsSync(path.join(absTarget, ".e2e"));
  if (features.has("e2e") && !ownsE2e) {
    features.delete("e2e");
    warn(
      `.e2e/ already exists and bearing did not create it — skipping the e2e module.\n` +
        `    Its harness would replace core/report.js and break every verify/*.js that imports the old names.\n` +
        `    To let bearing own it: move your .e2e/ aside, then re-run with --features +e2e.`,
    );
  }
  ...
  // PRECEDENCE, and the order is the whole fix: an explicit flag on THIS run, then whatever the
  // repo's own `.mcp.json` already says, and only then this machine's recorded choice. `.mcp.json`
  // is committed and team-shared while the manifest is gitignored and per-machine, so reading the
  // manifest first let one developer's local preference rewrite a shared file on every update —
  // observed in both directions on real repos, and two developers updating in turn would flip it
  // back and forth forever (see existingMcpTransport).
  const mcpTransport = parseMcpTransport(
    opts.mcpTransport ??
      existingMcpTransport(absTarget) ??
      readManifest(absTarget)?.data.mcpTransport ??
      STDIO_TRANSPORT,
  );
  ...
  const removed = [];
  const restored = [];
  const disowned = []; // ours to stop managing, never ours to delete
  ...
  for (const rel of previouslyOurs) {
    ...
    try {
      ...
      if (fs.existsSync(bak)) {
        ...
        fs.unlinkSync(bak);
        restored.push(rel);
      } else if (rel === NORTHSTARS_REL && hasAuthoredNorthStars(abs)) {
        // DISOWN, do not delete. Declining the module says "stop managing this", never "destroy
        // the invariants I wrote". Unlike gold practices there is no block of ours mixed in — once
        // a human has written an NS-# the whole file is theirs — so it is not even set aside, it
        // is simply left where it is and dropped from the manifest. Uninstall then cannot take it
        // either, because uninstall only removes what the manifest claims (NS-1: this project is
        // named after deleting exactly this file).
        disowned.push(rel);
      } else if (rel === GOLD_PRACTICES_REL && hasProjectPractices(abs)) {
        // THIS FILE HAS TWO OWNERS. Above the END marker is bearing's block; below it is the
        // project's own `PP-#` practices, which bearing never writes and which are earned the same
        // way north-stars are. Declining the module says "I do not want YOUR rules" — it cannot be
        // read as "delete mine". The file is gitignored, so an unlink is unrecoverable: no backup,
        // nothing in git. Set aside instead, under the convention the bundle copy already uses
        // when it meets a file it did not write (NS-1).
        fs.renameSync(abs, `${abs}.bearing-backup`);
        removed.push(rel);
        warn(
          `gold practices declined — your own PP-# rules were kept at ${rel}.bearing-backup`,
        );
      } else {
        fs.unlinkSync(abs);
        removed.push(rel);
  ...
  if (removed.length || disowned.length) {
    const gone = new Set([...removed, ...disowned]);
  ...
  // A REMOVED HOOK CANNOT FAIL OPEN. Every hook in this kit catches its own errors and exits 0 when
  // its libs are missing (NS-5), but that code never runs if the FILE is gone: node fails to
  // resolve the entry point and prints a raw MODULE_NOT_FOUND stack trace instead. Claude Code
  // reads `.claude/settings.json` once at session start, so an editor open during this update keeps
  // invoking the hooks we just deleted — on every single tool call, with a wall of stack traces and
  // nothing naming the cause. Observed on two machines. The registration on disk is already
  // correct, so there is nothing to repair; the only fix is a restart, and that is worth saying
  // plainly rather than leaving someone to decode a loader trace (NS-6).
  const removedHooks = removed.filter((rel) => /^\.claude\/hooks\/.*\.mjs$/.test(rel));
  if (removedHooks.length) {
    warn(
      `${removedHooks.length} Claude Code hook(s) were removed. Any session ALREADY OPEN in this ` +
        `repo still has them registered and will print MODULE_NOT_FOUND on every tool call until ` +
        `you restart it — the files on disk are correct, the running session is stale.`,
    );
  }
  for (const rel of disowned)
    ok(`kept your ${rel} — the module is off, but the north-stars in it are yours`);
  ...
  // Same tri-state. A stealth install with gitnexus fully selected was told "gitnexus module not
  // selected" — a user could reasonably re-run the installer to add a module they already have.
  ok(
    wantsScripts
      ? `package.json: ${scriptStats.added} added, ${scriptStats.updated} updated (${scriptStats.total} entries)`
      : !features.has("gitnexus")
        ? "npm scripts skipped (gitnexus module not selected)"
        : "npm scripts skipped (stealth — commands live in .bearing/commands.json)",
  );
  ...
  if (features.has("gitnexus")) {
    ...
    // WITH THE RECORDED BINARY, exactly like the npm scripts. This wrote the raw defaults, so an
    // operator who asked for a local build got it in package.json and `npx gitnexus@latest` in the
    // command table — and in a STEALTH install that table is the only mapping there is, so
    // `howToRun` handed the agent the wrong program. Same defect the npm-script path already
    // carries a check for; postcheck's `scripts_binary` reads package.json and returns null when
    // there are no bearing scripts, which is every stealth install (NS-21).
    // Substitute the SAME way the npm scripts do — `DEFAULT_GITNEXUS_CMD` is the literal
    // `npx gitnexus@latest` that appears in the script bodies, not the resolved binary name.
    // Splitting on the latter produced `npx /opt/mybuild/gitnexus@latest`, which is a second wrong
    // answer rather than the right one; reuse the one substitution that already exists.
    const commandTable = Object.fromEntries(
      Object.entries(GITNEXUS_NPM_SCRIPTS).map(([k, v]) => [
        k,
        gitnexusCmd ? v.split(DEFAULT_GITNEXUS_CMD).join(gitnexusCmd) : v,
      ]),
    );
    fs.writeFileSync(
      path.join(absTarget, ".bearing/commands.json"),
      `${JSON.stringify(commandTable, null, 2)}\n`,
    );
  ...
  const manifest = {
    ...
    stealth,
    createdGitignore,
    // Sticky: by the second run `.e2e/` exists BECAUSE WE MADE IT, so re-deriving would read our
    // own artifact as the user's and permanently disable the module (NS-22).
    ownsE2e,
    prettierIgnore,
  ...
  chmodScripts(absTarget, manifest.files ?? []);
  ...
  if (opts.runSetup !== false) {
    ...
    // TRI-STATE, not binary. `wantsScripts` is false for two different reasons — stealth (the files
    // ARE there, only the npm aliases are not) and no-gitnexus (there is no `scripts/` at all) —
    // and collapsing them told an intel-only or codex install to run a file it was never given:
    // `bash scripts/bearing-setup.sh` → ENOENT, exit 127. The runSetup path 25 lines up gates this
    // correctly; this branch never got the same treatment (NS-6, NS-20).
    warn(
      !features.has("gitnexus")
        ? "Nothing to set up — the gitnexus module was not selected, so there is no index to build."
        : wantsScripts
          ? "Run npm run bearing:setup in the target repo"
          : "Run bash scripts/bearing-setup.sh in the target repo (stealth: no npm scripts)",
    );
  }
...
/**
 * Make the shell scripts WE SHIPPED executable — and only those.
 *
 * This used to walk `scripts/`, `.cursor/hooks/` and `.githooks/` and chmod EVERY `.sh` it found to
 * 755, which in a real repository means the project's own scripts too: a plain `bearing update`
 * flipped three of one repo's files from 644 to 755 and put them in the diff the user then had to
 * explain. Changing the mode of a file we did not write is the same class of overreach as editing
 * its contents (NS-1), and it is worse for being invisible — a mode-only change shows no diff body,
 * so it reads as noise and gets committed without thought.
 *
 * The manifest already names exactly what bearing owns, so use it rather than a directory sweep.
 * @param {string} targetRoot @param {string[]} ownedFiles repo-relative paths bearing wrote
 */
function chmodScripts(targetRoot, ownedFiles) {
  for (const rel of ownedFiles) {
    if (!rel.endsWith(".sh")) continue;
    try {
      fs.chmodSync(path.join(targetRoot, rel), 0o755);
    } catch {
      /* ignore — a file we recorded but could not chmod must not fail the install */
    }
  }
}
...
export function inferInstallFromDisk(root) {
  ...
  // `.cursor/` on disk is now EVIDENCE OF A PAST INSTALL, not of a runtime to install. Deriving
  // "cursor" here handed parseRuntime a token this version cannot install, and every fresh clone of
  // a repo with a committed `.cursor/` rule died on it — the ordinary team case (NS-15, and the
  // same rule detect-runtime.mjs was given: a detector may only name runtimes the parser accepts).
  ...
  // Bearing's own file, present exactly when the module is. Detecting it directly matters even
  // though split-inheritance would also add it alongside northstars: a repo that chose gold
  // practices WITHOUT north-stars is not covered by that rule, and update deletes what is not
  // selected.
  if (has(".bearing/gold-practices.md")) features.push("goldpractices");
  ...
  // Either half is proof: the pack is the payload, the skill is how it is read.
  if (has(".bearing/lang/typescript.md") || skill("bearing-tsjs")) features.push("tsjs");
  if (has(".bearing/stack/frontend.md") || skill("bearing-frontend")) features.push("frontend");
  if (has(".bearing/stack/react.md") || skill("bearing-react")) features.push("react");
  // Shipped as a module on main without being taught here, so a manifest-less clone would have
  // had .e2e/ removed by its next update — the failure this whole function exists to prevent.
  if (has(".e2e/package.json")) features.push("e2e");  if (has(".mcp.json") || has(".bearing/lib/gitnexus-cmd.mjs")) features.push("gitnexus");
...
export function updateKit(targetRoot, opts = {}) {
  ...
  // A set read back from the manifest was recorded before some of today's modules existed, so it
  // inherits whatever has since been split out of what it selected — otherwise agreeing to a module
  // in an older version is what deletes its file in a newer one. Read it off `base`, never
  // `prev.data`: a repo with a tracked .bearing/ but no manifest is inferred from disk, and `prev`
  // is null on exactly that path.
  const inheritedBase = base.features
    ? [...inheritSplitFeatures(new Set(base.features))]
    : undefined;
  return installKit(absTarget, {
    ...
    //
    // Inheritance happens BEFORE the delta, not instead of it. `+tsjs` is not a full explicit
    // choice — it is an EDIT to the recorded set, and the recorded set is precisely the thing that
    // predates the split. Resolving the delta against a raw base made `update --features +tsjs`
    // a silent `-goldpractices` on every 1.1.x repo on disk, deleting .bearing/gold-practices.md
    // with the user's own PP-# rules in it (NS-1). A bare list still REPLACES and is honoured as
    // written, because that names every module with today's ids available.
    features:
      applyFeatureDelta(opts.features, inheritedBase) ??
      (inheritedBase && featureSpecOf(inheritedBase)),
    // ONLY an explicit flag. Falling back to `base.mcpTransport` here handed installKit the
    // manifest's value dressed up as an operator's choice, which then outranked the repo's own
    // committed `.mcp.json` — defeating the precedence installKit exists to apply. installKit
    // reads the manifest itself, so passing nothing loses nothing.
...
export function findInstalledRepos(searchRoot) {
  ...
  function walk(dir, depth = 0) {
    ...
    // A COMMITTED install whose manifest is gitignored is INVISIBLE here — and that is the ordinary
    // team case, not an edge case: a teammate clones a repo with 97 tracked `.bearing/` files and
    // `update-all` walks straight past it, because the single file discovery looks for is the one
    // file that is never committed. `updateKit` already falls back to `hasSharedInstall` for exactly
    // this situation and says so in its own comment; discovery was never wired to the same answer,
    // so the two disagreed about whether the same repo was installed.
    //
    // Discovery is held to a STRICTER standard than a targeted update, deliberately. Naming a path
    // is a decision the user made; a scan acts on whatever it matches. `.claude/hooks` and
    // `.cursor/rules` are tracked in plenty of repos that never installed this kit, and matching
    // those here would install bearing into someone's unrelated project (NS-1). So only bearing's
    // OWN namespaces count as proof of an install.
    if (fs.existsSync(path.join(dir, ".git"))) {
      const { shared, paths } = hasSharedInstall(dir);
      if (
        shared &&
        paths.some((p) => p.startsWith(".bearing/") || p.startsWith("scripts/bearing-"))
      ) {
        found.push(dir);
        return;
      }
...
/** @param {string} targetRoot */
/** Never let a missing directory abort a removal. @param {string} dir @returns {string[]} */
...
/**
 * Install (or remove) the status line — USER-WIDE, once, on purpose.
 *
 * Not part of `install`. A status line is one bar at the bottom of every session in every project,
 * so a per-repo install has no business writing one: project settings sit above user settings in
 * the precedence chain, so wiring it per repo would silently replace whatever the user had built
 * for themselves, in every repo that happened to have bearing in it.
 *
 * One copy covers everything. The script reads the project directory out of the session payload
 * Claude Code hands it on stdin, so it reports the right repo's index freshness wherever you are —
 * and in a repo with no bearing, that field simply prints nothing.
 *
 * Wired the same way a hand-written one is (`~/.claude/statusline.sh`, shebang + exec bit, no
 * interpreter prefix and no shell variable) so it carries none of #18's cross-shell problem.
 * @param {{ remove?: boolean }} [opts]
 */
export function installStatusLine(opts = {}) {
  const home = process.env.HOME || os.homedir();
  const dir = path.join(home, ".claude");
  const dest = path.join(dir, "statusline.sh");
  const settingsPath = path.join(dir, "settings.json");
  const command = "~/.claude/statusline.sh";
  const isOurs = (sl) => /statusline\.sh/.test(sl?.command ?? "");
  const cfg = readJsonSafe(settingsPath, {}) ?? {};

  // Where the status line we displaced is kept, so removal can put it back. A backup nobody can
  // find is not a backup — this path is printed on the way in AND on the way out.
  const backupPath = path.join(dir, "statusline.replaced.json");
  const readBackup = () => readJsonSafe(backupPath, null);

  if (opts.remove) {
    // Only ever our own, and only what we can prove we wrote (NS-1 — this is their home directory).
    if (isOurs(cfg.statusLine)) {
      const previous = readBackup();
      if (previous) {
        // PUT THEIRS BACK. Removing bearing's line and leaving them with none is not "as we found
        // it" — they had a status line before we replaced it, and the whole justification for
        // replacing by default is that this is reversible.
        ...
        ok(`Restored the status line bearing replaced (${previous.command}).`);
      } else {
        delete cfg.statusLine;
        fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(settingsPath, `${JSON.stringify(cfg, null, 2)}\n`);
        ok("Removed bearing's status line from ~/.claude/settings.json");
      }
    } else if (cfg.statusLine) {
      warn("Left your status line alone — bearing did not write it.");
    }
    try {
      fs.unlinkSync(dest);
  ...
  fs.mkdirSync(dir, { recursive: true });
  fs.copyFileSync(path.join(BUNDLE_ROOT, "templates/statusline.sh"), dest);
  fs.chmodSync(dest, 0o755);
  ok(`Installed ${dest}`);

  // REPLACE BY DEFAULT, BUT NEVER WITHOUT A WAY BACK. An earlier version refused and printed the
  // line to paste, which is the safe-looking choice and the wrong one: `bearing statusline` is an
  // explicit request, and answering it with homework means the feature does not happen. What made
  // refusing tempting is real though — overwriting is destructive AND invisible, since you find out
  // by noticing the bottom of your screen changed. So the answer is not to refuse, it is to make it
  // reversible and say so: the displaced config is written out verbatim, the path is printed, and
  // `--remove` puts it back rather than leaving you with nothing (NS-1, NS-6).
  //
  // Only the SETTINGS entry is saved. Their script itself is never touched — bearing writes
  // `statusline.sh` and a `statusline.py` beside it is still theirs, still on disk, still working.
  if (cfg.statusLine && !isOurs(cfg.statusLine)) {
    fs.writeFileSync(backupPath, `${JSON.stringify(cfg.statusLine, null, 2)}\n`);
    warn(
      `Replaced your status line (${cfg.statusLine.command}).\n` +
        `    Saved to ${backupPath} — \`bearing statusline --remove\` puts it back.\n` +
        `    Your own script is untouched; only the settings entry changed.`,
    );
  }
  // Keep every option they had set — refreshInterval, hideVimModeIndicator and anything Claude Code
  // adds later. Only the command is ours to change.
  cfg.statusLine = { ...cfg.statusLine, type: "command", command };
  fs.writeFileSync(settingsPath, `${JSON.stringify(cfg, null, 2)}\n`);
  ok(`Wired it in ${settingsPath} — restart Claude Code to see it.`);
}
...
export function uninstallKit(targetRoot, opts = {}) {
  ...
  // `opts.runtime` is honoured here for the same reason update honours it: a manifest can record a
  // runtime this version cannot install, and removal must never be the one operation with no way
  // out (NS-6). parseRuntime drops retired tokens, so this no longer throws either way.
  const runtime = parseRuntime(opts.runtime ?? manifest.runtime ?? "both");
  ...
  for (const rel of manifest.files ?? []) {
    ...
    try {
      ...
      if (fs.existsSync(bak)) {
        ...
        // NS-1 IS NAMED AFTER THIS. "Uninstall once deleted the user's north-stars because
        // `.bearing/` looked kit-owned" — and the moment bearing began seeding a starter, the file
        // entered the manifest and this loop would have done it again, to the one artifact the
        // whole product exists to protect. The keep-check further down still says north-stars
        // "always survive"; this is what makes that true rather than merely written.
        keptOnRemove.push(rel);
  ...
  for (const rel of keptOnRemove)
    console.log(`  kept ${rel} — your north-stars outlive the tool that read them`);
  ...
  // WRAPPED. A bare readdir here threw ENOENT on a repo whose `.bearing/` the user had already
  // deleted by hand — a very plausible first attempt at removing bearing — and took the whole
  // uninstall with it, BEFORE the npm scripts, the gitignore block, the `.git/info/exclude` block,
  // the adapter unwire and the manifest deletion. The repo was then left with hooks registered, MCP
  // configured and a CLAUDE.md block intact, still reporting as installed. The identical read 20
  // lines down already has a try/catch (NS-1).
  for (const f of safeReaddir(path.join(absTarget, ".bearing")).filter((n) => /^\.bearing-|^\.gitnexus-/.test(n))) {
    try {
  ...
  // The same cleanup `update` runs. It lived only in the migration path, and uninstall never calls
  // migration — so `bearing uninstall` on a Cursor-era install left `.cursor/mcp.json` still
  // pointing a dead runtime at the GitNexus MCP server. Only OUR entry goes; a user's own servers
  // in that file stay (NS-1).
  cleanupRetiredCursor(absTarget, []);
...
export async function cliMain(argv, invokedAs = "node lib/kit.mjs") {
  ...
  ${invokedAs} statusline [--remove]   Status line for Claude Code — USER-WIDE, one time, not per repo
  ...
  // Machine-wide, so it never wants a target — dispatched before the "missing target" guard.
  if (cmd === "statusline") {
    installStatusLine({ remove: flags.has("--remove") });
    return;
  }
  ...
  // Dropping a retired runtime silently would be its own NS-20 problem: the user asked for
  // something and got something else with no line explaining it. parseRuntime is deliberately quiet
  // (it is also the reader for recorded state); saying so belongs here, at the input.
  const typedRetired = String(resolvedRuntime || "")
    .toLowerCase()
    .split(",")
    .map((t) => t.trim())
    .filter((t) => RETIRED_RUNTIMES.includes(t));
  if (typedRetired.length) {
    warn(
      `${typedRetired.join(", ")} is no longer a bearing runtime — support was removed in 1.2.0.\n` +
        `    Installing for: ${parseRuntime(resolvedRuntime)}. Enforcement needs tool-interception hooks, and only Claude Code has them.`,
    );
  }
