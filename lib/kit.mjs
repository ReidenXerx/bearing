...
function printInstallComplete(
  ...
  summaryTable({
    ...
    rows: [
      ...
      { label: "Branch", value: currentBranch(absTarget) ?? "(detached)", status: "info" },
...
export async function cliMain(argv, invokedAs = "node lib/kit.mjs") {
  ...
  if (!cmd || cmd === "--help" || cmd === "-h") {
    console.log(`Usage:
  ${invokedAs} install <target-repo> [--runtime cursor|zed|claude|both|all] [--features ${FEATURE_IDS.join(",")}|all] [--repo-name NAME] [--stealth] [--mcp http|stdio|PORT] [--gitnexus-cmd CMD] [--quick] [--no-setup] [--skip-verify]
    ...
    if (detected.length) {
      ...
      warn(
        `No agent environment detected and no TTY to ask — installing for "both" (cursor+zed).\n` +
          `    If you use Claude Code or Codex: ${KIT_NAME} update ${target} --runtime claude`,
      );
    }
  }
