...
import { parseMcpTransport, STDIO_TRANSPORT } from "./mcp-config.mjs";
...
export function installKit(targetRoot, opts = {}) {
  ...
  const gitnexusCmd =
    opts.gitnexusCmd ?? readManifest(absTarget)?.data.gitnexusCmd ?? undefined;
  ...
  for (const adapter of adapters) {
    ...
    const wrote = adapter.wire(absTarget, { repoName, features, mcpTransport });
