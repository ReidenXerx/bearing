...
export function installKit(targetRoot, opts = {}) {
  ...
  const features = opts.features
    ...
    : recordedFeatures
      ? inheritSplitFeatures(parseFeatures(recordedFeatures.join(",")))
...
export function updateKit(targetRoot, opts = {}) {
  ...
  return installKit(absTarget, {
    ...
    //
    // And when nobody said anything, the recorded set inherits whatever has since been split out of
    // what it selected: without that, a repo that agreed to `northstars` in 1.1.x has
    // .bearing/gold-practices.md deleted by the next update. Read it off `base`, never
    // `prev.data` — a repo with a tracked .bearing/ but no manifest is inferred from disk, and
    // `prev` is null on exactly that path.
    features:
      applyFeatureDelta(opts.features, base.features) ??
      (base.features
        ? [...inheritSplitFeatures(new Set(base.features))].join(",")
        : undefined),      mcpTransport: opts.mcpTransport ?? base.mcpTransport,
