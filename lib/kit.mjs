 ...
 */
function copyBundleFile(src, dest, repoName, persona) {
  fs.mkdirSync(path.dirname(dest), { recursive: true });
  if (isTextCandidate(src)) {
    fs.writeFileSync(
      dest,
      substitutePlaceholders(fs.readFileSync(src, "utf8"), { repoName, persona }),
    );
  } else {
...
export function installKit(targetRoot, opts = {}) {
  ...
  if (domain.created) {
    ok(`Domain: ${domain.domain ?? "general"} — reviewing as ${domain.persona}`);
    if (domain.evidence.length) ok(`  inferred from ${domain.evidence.join(", ")}`);
    if (domain.suggested) {
      ok(`  signals hinted at "${domain.suggested}" but not strongly enough to adopt it`);
    }
    // Say it plainly rather than let a wrong guess quietly bias every later judgement.
    warn(`  wrong domain? edit ${DOMAIN_PATH} — bearing will not overwrite it`);
  } else {
    ok(`Domain: reviewing as ${domain.persona} (pinned in ${DOMAIN_PATH})`);
  }
