...
/**
 * The agent docs THIS install WRITES — which is not the same as the agent docs that exist.
 *
 * bearing writes `CLAUDE.md` for the claude runtime and `AGENTS.md` for zed/codex. A repo very
 * easily holds the other one: committed by the team, or left behind by an earlier `--runtime all`
 * install that was later narrowed. Two checks below asserted over both files whenever they merely
 * EXISTED, so a healthy `runtime: claude` repo with a tracked AGENTS.md reported
 *
 *   ✗ domain persona reached the contract: no persona line in AGENTS.md
 *   ✗ one managed block per agent doc: AGENTS.md: 0 current + 0 legacy block(s)
 *
 * and every update of it ended "Update finished with 2 FAILED checks" — for a file bearing had
 * correctly declined to touch. That inverts this module's whole purpose: the headline became the
 * false claim (NS-20), and a check that fails on a correct install is worse than no check (NS-5).
 *
 * Falls back to BOTH docs when the runtime is unknown or unreadable, because a missing answer must
 * never narrow verification — that is how a check quietly stops checking (NS-21).
 *
 * @param {any} m the manifest, or null
 * @returns {string[]}
 */
function managedAgentDocs(m) {
  const ids = runtimeIds(m?.runtime || "");
  if (!ids.size) return ["CLAUDE.md", "AGENTS.md"];
  const docs = [];
  if (ids.has("claude")) docs.push("CLAUDE.md");
  if (ids.has("zed") || ids.has("codex")) docs.push("AGENTS.md");
  return docs.length ? docs : ["CLAUDE.md", "AGENTS.md"];
}
...
function checkCommandsRun(root, features) {
  ...
  for (const c of SAFE) {
    ...
    for (const other of ["cursor", "zed", "codex"]) {
      ...
      const line = out
        .split("\n")
        // WORD-BOUNDED. `new RegExp("zed","i")` matches "analy**zed**", "initiali**zed**" and
        // "recogni**zed**" — so a perfectly ordinary "✗ index not analyzed yet" read as "complains
        // about zed, which is not installed" and failed the install with a false alarm (NS-5).
        .find((l) => /[✗]/.test(l) && new RegExp(`\\b${other}\\b`, "i").test(l));
...
function checkSingleManagedBlock(root, stealth = false, docs = ["AGENTS.md", "CLAUDE.md"]) {
  ...
  return bad.length
    ...
    : { id: "agent_docs", ok: true, label: "one managed block per agent doc", detail: docs.join(" / ") };
}
...
function guardsPath(body, target) {
  ...
  // COMMENTS ARE NOT CODE. A guard quoted in a `#` comment — "we used to do: [[ -f x ]] || exit" —
  // satisfied this while the real call below it was unconditional. Same for a `//` line and a
  // heredoc of prose. Strip them before looking.
  const code = body
    .split("\n")
    .filter((l) => !/^\s*(?:#|\/\/|\*)/.test(l))
    .join("\n");
  // ANCHORED AT THE END. The literal was unterminated, so a guard on any LONGER path sharing the
  // prefix counted — `[[ -f scripts/ghost.mjs.bak ]]` satisfied a check for `scripts/ghost.mjs`.
  // An optional `$VAR/` prefix is accepted because bearing's own scripts set `ROOT=` and then test
  // "$ROOT/x"; without it a correctly guarded reference read as dangling, which is the false alarm
  // NS-5 weighs heaviest.
  const end = `(?=["'\\s\\]])`;
  const prefix = `(?:\\$\\{?[A-Za-z_][\\w]*\\}?\\/)?`;
  return new RegExp(
    `(?:\\[\\[?\\s*!?\\s*-[a-z]\\s+["']?${prefix}${lit}${end}` +
      `|existsSync\\(\\s*["'\`]${prefix}${lit}${end})`,
  ).test(code);
}
 ...
 */
function checkPersonaResolved(root, stealth = false, managed = ["CLAUDE.md", "AGENTS.md"]) {
  ...
  const docs = (stealth ? [".bearing/contract.md"] : managed).filter((f) => exists(root, f));
...
export function runPostChecks(root, ctx = {}) {
  ...
  return [
    ...
    checkPersonaResolved(root, Boolean(m?.stealth), managedAgentDocs(m)),
    checkStealthInvisible(root, m),
    checkNoLegacyArtifacts(root),
    checkSingleManagedBlock(root, Boolean(m?.stealth), managedAgentDocs(m)),
