...
function checkCommandsRun(root, features) {
  ...
  // Which runtimes this install actually covers, expanded the same way the installer expands them.
  const installed = new Set();
  for (const t of String(manifestRuntime(root) || "").toLowerCase().split(",").map((x) => x.trim()).filter(Boolean)) {
    if (t === "both") { installed.add("cursor"); installed.add("zed"); }
    else if (t === "all") { installed.add("cursor"); installed.add("zed"); installed.add("claude"); installed.add("codex"); }
    else installed.add(t);
  }
  for (const c of SAFE) {
...
function checkPersonaResolved(root, stealth = false) {
  ...
  const docs = (
    stealth
      ? [".bearing/contract.md", ".cursor/rules/00-bearing-enforcement.mdc"]
      : ["CLAUDE.md", "AGENTS.md", ".cursor/rules/00-bearing-enforcement.mdc"]
