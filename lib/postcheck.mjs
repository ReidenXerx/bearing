...
function checkMachineLocalIgnored(root) {
  ...
  const leaked = mustIgnore.filter((rel) => {
  ...
  return {
    ...
    detail: leaked.length ? `would be committed: ${leaked.join(", ")}` : "manifest, session state, task-core, backups",
  };
}
...
function checkPersonaResolved(root) {
  ...
  const leaked = docs.filter((f) => (read(root, f) ?? "").includes("__BEARING_PERSONA__"));
