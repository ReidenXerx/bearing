...
export function migrateLegacyInstall(absTarget, runtime) {
  ...
  cleanupMemoryIndex(absTarget, actions);
...
/**
 * Take bearing's own checkpoint stanzas back OUT of Claude Code's memory index.
 *
 * The PreCompact hook used to append two lines to `~/.claude/projects/<slug>/memory/MEMORY.md` on
 * every compaction. That file is the INDEX — loaded into context every session, one pointer line
 * per memory — so the stanzas cost window forever and buried the real entries. One project
 * accumulated nine around a single genuine pointer. The write is gone; this removes what it left.
 *
 * DELIBERATELY NARROW (NS-1 — this is the user's file, outside the repo). Only a `<!-- checkpoint
 * <ISO> -->` marker followed by lines bearing itself wrote is removed; a stanza the user has edited
 * or annotated keeps its marker but not our shape, and stays. The header goes only when it matches
 * ours byte-for-byte, because a user who rewrote it meant it.
 * @param {string} absTarget @param {string[]} actions
 */
function cleanupMemoryIndex(absTarget, actions) {
  const home = process.env.HOME || os.homedir();
  const slug = path.resolve(absTarget).replace(/\//g, "-");
  const file = path.join(home, ".claude", "projects", slug, "memory", "MEMORY.md");
  let text;
  try {
    text = fs.readFileSync(file, "utf8");
  } catch {
    return; // no index here — nothing bearing ever wrote
  }
  // BOTH lines required. With them optional, a marker followed by anything starting `- trigger:`
  // was consumed — including a line a user had rewritten. Our stanza is exactly these three lines;
  // anything else keeps the marker but not the shape, and is left alone.
  const OURS = /\n?<!-- checkpoint \d{4}-\d{2}-\d{2}T[\d:.]+Z -->\n- trigger: [^\n]*\n- \(transcript about to be summarized[^\n]*\n/g;
  let out = text.replace(OURS, "");
  // Both header spellings bearing has written, and only when otherwise unedited.
  for (const kit of ["bearing", "GitNexus kit"]) {
    out = out.replace(
      `# Project working memory (${kit})\n\n> Durable across compaction + sessions. Keep this current: task, decisions, findings, open items, key file:line. Nothing important should live only in the volatile transcript.\n`,
      "",
    );
  }
  out = out.replace(/\n{3,}/g, "\n\n").replace(/^\n+/, "");
  if (out === text) return;
  const removed = (text.match(/<!-- checkpoint /g) || []).length - (out.match(/<!-- checkpoint /g) || []).length;
  try {
    // If nothing of the user's is left, the file is entirely ours and an empty index is litter.
    if (out.trim()) fs.writeFileSync(file, out);
    else fs.unlinkSync(file);
    actions.push(
      `memory: removed ${removed} bearing checkpoint stanza(s) from Claude Code's MEMORY.md index`,
    );
  } catch {
    /* their file, their permissions — never fail an install over it */
  }
}
