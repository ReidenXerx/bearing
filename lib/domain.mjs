/**
 * DOMAIN PERSONA — what kind of expert is reviewing this repository.
 *
 * The product's headline claim is that a trading repo gets reviewed by a quant trader and a
 * payments repo by a ledger engineer, because domain expertise is what catches SEMANTIC wrongness:
 * "this fee is computed on gross, should be net" is invisible to a language-only reviewer.
 *
 * That claim was implemented in exactly one skill. `.bearing/domain.json` appeared in the README
 * and in bearing-microscope, and nowhere else — nothing created it, nothing seeded it, and no other
 * skill, contract or session brief read it. Two consequences:
 *
 *   1. Every review skill except microscope worked as a generic engineer.
 *   2. Microscope re-inferred the domain from scratch on every wave, so wave 2 could adopt a
 *      different persona than wave 1 while folding in wave 1's findings.
 *
 * So the persona is resolved ONCE at install, written to a file the user owns, and injected into
 * the always-on contract where every skill and every session sees it.
 *
 * On being wrong: a confidently wrong persona is worse than a generic one, because it biases every
 * judgment downstream. Inference therefore reports its evidence, falls back to a neutral persona
 * when signals are weak, and the installer prints what it chose so it can be corrected. The file is
 * never overwritten once it exists — it is the user's to edit (NS-1).
 */
import fs from "node:fs";
import path from "node:path";

import { agentsBlockSource } from "./constants.mjs";

export const DOMAIN_PATH = ".bearing/domain.json";

/**
 * Signal → domain. Ordered: the first match wins, so put the specific before the generic
 * (a "trading platform API" is a trading repo, not a generic API repo).
 *
 * Each persona names a SENIORITY and a SPECIALISM, because "senior engineer" alone reproduces the
 * generic reviewer this exists to replace.
 */
const DOMAINS = [
  {
    id: "trading",
    persona: "senior quantitative trader and trading-systems engineer",
    // `pnl` and `slippage` are near-unambiguous; `order`/`position` alone are not, so they only
    // count alongside another signal (see scoring below).
    strong: /\b(backtest|slippage|pnl|order book|orderbook|market data|candlestick|ohlcv|quant|alpha signal|take profit|stop loss)\b/i,
    weak: /\b(trading|trade|exchange|broker|portfolio|position|ticker|binance|kraken|futures|derivative)\b/i,
  },
  {
    id: "payments",
    persona: "staff payments and ledger engineer",
    strong: /\b(double[- ]entry|ledger|chargeback|settlement|reconcil\w+|payout|interchange|pci[- ]dss)\b/i,
    weak: /\b(payment|invoice|billing|stripe|checkout|refund|subscription|currency)\b/i,
  },
  {
    id: "healthcare",
    persona: "clinical systems engineer with HIPAA and patient-safety expertise",
    strong: /\b(hipaa|phi\b|fhir|hl7|icd-?10|ehr|emr|clinical trial|healthcare)\b/i,
    // `health\w*`, not `health` — \b after "health" fails on "healthcare", so a package described
    // as a "Healthcare … Application" scored ZERO for healthcare and lost to a passing mention of
    // something else. A repo naming its own domain must be the strongest signal there is, which is
    // why "healthcare" is also promoted to strong above.
    // Claims vocabulary is WEAK on purpose: `claim`, `insurance`, `payer` appear in insurtech and
    // fintech too. One mention must not brand a repo — but two or more distinct graph AREAS naming
    // them is the product, and the counting rule above promotes that on its own.
    weak: /\b(patient|clinic\w*|medical|health\w*|diagnos\w+|prescription|provider|claim\w*|insurance|deductible|copay\w*|remittance|eob)\b/i,
  },
  {
    id: "identity",
    persona: "staff identity and access-management engineer",
    strong: /\b(oauth2?|oidc|saml|jwt|mfa|scim|refresh token|access token)\b/i,
    weak: /\b(auth\w*|identity|login|session|permission|rbac|tenant)\b/i,
  },
  {
    id: "ml",
    persona: "senior ML systems engineer",
    strong: /\b(embedding|fine[- ]tun\w+|inference server|training loop|vector (db|store|index)|rag\b|prompt engineering)\b/i,
    weak: /\b(machine learning|\bml\b|model|dataset|llm|neural|pytorch|tensorflow|openai|anthropic)\b/i,
  },
  {
    id: "infrastructure",
    persona: "staff platform and reliability engineer",
    strong: /\b(kubernetes|terraform|helm|service mesh|autoscal\w+|sre\b|observability)\b/i,
    weak: /\b(infra\w*|deploy\w*|docker|cluster|provision\w*|pipeline|ci\/cd)\b/i,
  },
  {
    id: "developer-tooling",
    persona: "senior developer-tooling engineer",
    strong:
      /\b(language server|code[- ](graph|index|review)|static analysis|ast\b|linter|codemod|mcp\b|developer[- ]tools?|context[- ]engineering|agentic[- ]ai)\b/i,
    weak: /\b(devtool|scaffold\w*|generator|compiler|parser|agent|editor|\bcli\b)\b/i,
  },
  {
    id: "e-commerce",
    persona: "senior commerce-platform engineer",
    strong: /\b(shopping cart|product catalog|fulfil?lment|sku\b|merchandis\w+)\b/i,
    weak: /\b(ecommerce|e-commerce|storefront|shop|inventory|order management)\b/i,
  },
  {
    id: "data-acquisition",
    persona: "senior data-acquisition and pipeline engineer",
    strong: /\b(web scrap\w+|crawler|etl\b|data pipeline|ingest\w+ pipeline)\b/i,
    weak: /\b(scrape\w*|crawl\w*|lead gen\w*|enrich\w*|dataset build)\b/i,
  },
];

/** The honest default: a real seniority, no invented specialism. */
export const NEUTRAL_PERSONA = "senior software engineer experienced in this project's stack";

/**
 * Read up to `n` bytes of a text file, lowercased, with OUR OWN managed block removed.
 *
 * Stripping it is not tidiness, it is correctness. bearing injects a contract into CLAUDE.md and
 * AGENTS.md that talks at length about MCP servers, code graphs, impact analysis and linters —
 * every one of them a developer-tooling signal. Inference then read that back as evidence about
 * the repository. Observed on a real health platform with no package description: the only signal
 * found was `CLAUDE.md: "mcp"`, from bearing's own text, and it suggested "developer-tooling".
 *
 * Low confidence saved it from adopting the wrong persona, but a repo with slightly more of its own
 * vocabulary would have tipped over. A tool must not treat its own output as the world's input.
 */
function readText(root, rel, n = 8000) {
  let raw;
  try {
    raw = fs.readFileSync(path.join(root, rel), "utf8");
  } catch {
    return ""; // absent is normal — README/CLAUDE.md are optional
  }
  // Deliberately OUTSIDE the try. A missing file is expected; a broken strip is a bug, and
  // swallowing it here made every source read as empty — inference silently found nothing and
  // fell back to the neutral persona, which looks exactly like an honest "no signal".
  //
  // Two blocks come out, not one. Besides bearing's own contract, `gitnexus analyze` writes a
  // stats block into CLAUDE.md ("indexed by GitNexus … use the GitNexus MCP tools to …") which sits
  // OUTSIDE our markers. Stripping only ours still left that, and it alone was enough to suggest
  // "developer-tooling" for a claims platform. Anything a tool wrote about itself is not the
  // repository describing itself.
  return raw
    .replace(new RegExp(agentsBlockSource(), "gm"), " ")
    .replace(/<!--\s*gitnexus:start\s*-->[\s\S]*?<!--\s*gitnexus:end\s*-->/gm, " ")
    .slice(0, n)
    .toLowerCase();
}

/**
 * Infer the domain from what the repository says about itself.
 *
 * Deliberately reads only self-descriptions — package metadata, README, CLAUDE.md — rather than
 * scanning source. Source scanning finds the vocabulary of whatever library is in fashion; a
 * project's own description is what its authors think it IS.
 *
 * @param {string} root
 * @returns {{ domain: string|null, persona: string, confidence: 'high'|'low'|'none', evidence: string[] }}
 */
export function inferDomain(root) {
  /** @type {Record<string,string>} */
  const sources = {};
  try {
    const pkg = JSON.parse(fs.readFileSync(path.join(root, "package.json"), "utf8"));
    // Name, description and keywords only — the authors' own words about what this IS.
    //
    // Dependencies are deliberately EXCLUDED. They describe implementation, not domain, and
    // treating them as self-description is the same mistake as scanning source. Observed on a real
    // repo whose description reads "Healthcare Screen Capture and Voice Recording Application":
    // a `jwt` dependency scored as a STRONG identity signal in package.json, which counts as the
    // repo declaring itself, so it was adopted with high confidence and overrode the description.
    // Nearly every web app depends on a JWT, crypto or auth library; almost none of them are
    // identity products.
    sources["package.json"] = [pkg.name, pkg.description, ...(pkg.keywords ?? [])]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
  } catch {
    /* no package.json — other sources still apply */
  }
  sources["README.md"] = readText(root, "README.md");
  sources["CLAUDE.md"] = readText(root, "CLAUDE.md", 4000);
  // The graph's own area labels — derived FROM the code rather than written about it. Each label is
  // repeated by size so a 300-symbol area outweighs a 5-symbol one, without adding a second scoring
  // path: the matcher counts one hit per source, and this keeps that shape.
  const areas = graphAreas(root);
  if (areas.length) {
    sources["graph areas"] = areas
      .map((a) => `${a.label} `.repeat(Math.min(4, 1 + Math.floor(a.symbols / 40))))
      .join(" ")
      .replace(/-/g, " ")
      .toLowerCase();
  }

  let best = null;
  for (const d of DOMAINS) {
    const evidence = [];
    let score = 0;
    let declared = false; // a strong signal in the package's OWN metadata
    let anyStrong = false; // an unambiguous term anywhere (HIPAA, double-entry, order book…)
    for (const [name, text] of Object.entries(sources)) {
      if (!text) continue;
      const s = text.match(d.strong);
      const w = text.match(d.weak);
      // package.json name/description/keywords are the authors DECLARING what this is. README prose
      // merely discusses things — including other domains, as examples. Weighting them equally made
      // this repo classify as "trading" purely because its README explains the feature using a
      // trading example, which would then have written "quantitative trader" into its own contract.
      // package.json is the authors DECLARING what this is. Graph areas are derived from the code
      // itself, which beats README prose — prose discusses things, including other domains as
      // examples — but does not outrank the package's own statement of identity.
      const weight = name === "package.json" || name === "graph areas" ? 2 : 1;
      // GRAPH AREAS ARE COUNTED, NOT CONCATENATED. How many DISTINCT areas point at a domain is the
      // signal — one is a feature, several are the product.
      //
      // Promoting any single area hit to "strong" was too blunt in both directions. It rescued a
      // scraping product that a stray `JWT` in its README had branded `identity`, and in the same
      // pass it called an events platform a payments product because one area is named `stripe`.
      // An integration gets one area; the thing the codebase is FOR gets several — `upwork-scraper`,
      // `extraction`, `ingestion`, `enrichment`, `leads` all point one way.
      const graphMatches =
        name === "graph areas"
          ? areas.filter((a) => {
              const t = a.label.replace(/-/g, " ").toLowerCase();
              return d.strong.test(t) || d.weak.test(t);
            }).length
          : 0;
      const graphHit = graphMatches >= 2;
      if (s || graphHit) {
        score += 2 * weight;
        anyStrong = true;
        if (name === "package.json" || name === "graph areas") declared = true;
        evidence.push(`${name}: "${(s || w)[0]}"`);
      } else if (w) {
        score += weight;
        evidence.push(`${name}: "${w[0]}"`);
      }
    }
    if (score >= 2 && (!best || score > best.score)) best = { ...d, score, declared, anyStrong, evidence };
  }

  if (!best) {
    // Carry the area names out even when nothing matched. They are the best description of the
    // product that exists, and whoever answers "what is this project" should be handed them rather
    // than told the question is unanswerable.
    return {
      domain: null,
      persona: NEUTRAL_PERSONA,
      confidence: "none",
      evidence: [],
      suggested: null,
      areas: areas.map((a) => a.label),
    };
  }
  // Confident only when the repo says so itself, or when two independent sources agree strongly.
  // Confident when the repo declares it, or when an unambiguous term (HIPAA, double-entry, order
  // book) appears AND is corroborated. A single strong term in prose is not enough on its own — a
  // doc that merely discusses a domain must not brand the repo as that domain.
  const confident = best.declared || (best.anyStrong && best.score >= 4);
  if (!confident) {
    // Keep the guess visible so the user can accept it in one edit, but do NOT adopt it: a wrong
    // specialism skews every judgement downstream, and "senior engineer" at least skews nothing.
    return {
      domain: null,
      persona: NEUTRAL_PERSONA,
      confidence: "low",
      evidence: best.evidence.slice(0, 3),
      suggested: best.id,
    };
  }
  return {
    domain: best.id,
    persona: best.persona,
    confidence: "high",
    evidence: best.evidence.slice(0, 3),
    suggested: null,
  };
}

/** @param {string} root @returns {{domain:string|null,persona:string}|null} */
export function readDomain(root) {
  try {
    const d = JSON.parse(fs.readFileSync(path.join(root, DOMAIN_PATH), "utf8"));
    if (typeof d?.persona === "string" && d.persona.trim()) return d;
  } catch {
    /* absent or malformed → caller infers */
  }
  return null;
}

/**
 * Resolve the persona for this repo, writing the file on first install.
 *
 * NEVER overwrites: once the file exists it is the user's, and silently replacing a persona they
 * corrected would be the same class of bug as reverting their hooks.json (NS-1). Returns what was
 * resolved plus whether it was newly written, so the installer can show it for correction.
 *
 * @param {string} root
 * @returns {{ domain: string|null, persona: string, confidence: string, evidence: string[], created: boolean, source: 'file'|'inferred' }}
 */
export function ensureDomain(root) {
  const existing = readDomain(root);
  if (existing) {
    return {
      domain: existing.domain ?? null,
      persona: existing.persona,
      confidence: existing.confidence ?? "pinned",
      evidence: [],
      created: false,
      source: "file",
    };
  }
  const inferred = inferDomain(root);
  const body = {
    // Written for a human to edit: the comment is part of the contract with the user.
    _comment:
      "Domain persona adopted by bearing's review skills and injected into the always-on contract. " +
      "Edit `persona` to anything you like — bearing never overwrites this file once it exists.",
    domain: inferred.domain,
    persona: inferred.persona,
    confidence: inferred.confidence,
    inferredFrom: inferred.evidence,
    // Present only when the signals leaned somewhere but not far enough to adopt. Named so the
    // user can accept the guess by moving it into `persona`, rather than having it imposed.
    ...(inferred.suggested
      ? {
          suggestedDomain: inferred.suggested,
          graphAreas: inferred.areas?.length ? inferred.areas.slice(0, 12) : undefined,
          _suggestion: `Signals hinted at "${inferred.suggested}" but not strongly enough to adopt it. If that is right, set persona accordingly.`,
        }
      : {}),
  };
  try {
    fs.mkdirSync(path.join(root, path.dirname(DOMAIN_PATH)), { recursive: true });
    fs.writeFileSync(path.join(root, DOMAIN_PATH), JSON.stringify(body, null, 2) + "\n");
  } catch {
    // Not fatal: the skills fall back to inferring at runtime, which is the old behaviour.
    return { ...inferred, created: false, source: "inferred" };
  }
  return { ...inferred, created: true, source: "inferred" };
}

/**
 * Area labels the GRAPH found, read straight off disk.
 *
 * Inference reads package.json, the README and CLAUDE.md — prose ABOUT the code. When the repo is
 * indexed, a description derived FROM the code is already sitting there: gitnexus writes one skill
 * per detected community, and its frontmatter carries the area name and how many symbols it holds.
 * Establish it from the thing that defines it, not from something that describes it (GP-14).
 *
 * Observed: a repo whose package.json says nothing useful resolved to `domain: null` and a generic
 * persona, while its graph had already labelled it `upwork-scraper`, `leads`, `enrichment`,
 * `extraction`, `vollna` — a data-acquisition product, named unambiguously.
 *
 * Bearing's and gitnexus's own areas are skipped: they describe the tooling, not the product, and
 * they appear in every indexed repo that installed either.
 * @param {string} root
 * @returns {{ label: string, symbols: number }[]} biggest first
 */
const GENERIC_AREAS = new Set([
  "auth", "session", "sessions", "config", "configuration", "components", "component", "scripts",
  "script", "utils", "util", "helpers", "shared", "common", "core", "lib", "types", "models",
  "model", "stores", "store", "api", "apis", "routes", "controllers", "services", "middleware",
  "logger", "logging", "errors", "testing", "tests", "test", "fixtures", "migrations", "db",
  "database", "ui", "styles", "assets", "hooks", "context", "providers", "constants", "settings",
  "automation", "details", "data-table", "badge",
]);

export function graphAreas(root) {
  const out = [];
  for (const base of [".claude/skills", ".bearing/skills"]) {
    let entries = [];
    try {
      entries = fs.readdirSync(path.join(root, base));
    } catch {
      continue;
    }
    for (const dir of entries) {
      if (!dir.startsWith("gitnexus-area-")) continue;
      const label = dir.slice("gitnexus-area-".length);
      // `cluster-47` is the analyzer saying it could not name the group; tooling areas are ours.
      if (/^cluster[-_]?\d+$/.test(label) || /(bearing|gitnexus)-teaching/.test(label)) continue;
      // NEAR-UNIVERSAL AREAS SAY NOTHING ABOUT THE DOMAIN. This is the dependency lesson again:
      // almost every application has an `auth` area and almost none are identity products. Fed in
      // raw, `auth` alone branded a lead-scraping product as `identity` on its first run.
      if (GENERIC_AREAS.has(label)) continue;
      let symbols = 0;
      try {
        const fm = fs.readFileSync(path.join(root, base, dir, "SKILL.md"), "utf8").slice(0, 400);
        symbols = Number(fm.match(/(\d+)\s+symbols?/)?.[1]) || 0;
      } catch {
        /* a skill dir with no SKILL.md — still counts as a label */
      }
      if (!out.some((a) => a.label === label)) out.push({ label, symbols });
    }
  }
  return out.sort((a, b) => b.symbols - a.symbols);
}
