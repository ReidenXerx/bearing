 ...
 *
 * Detection may only ever name a runtime the installer still accepts. Cursor stayed in this list
 * after support for it was removed, so a `.cursor/` directory — which any repo that has been opened
 * in Cursor once still carries — made this return "cursor" and the install then died on
 * `Invalid runtime "cursor"`. A detector is an input to VALID_RUNTIMES, never a superset of it.
 *
