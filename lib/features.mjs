...
const CLAUDE_HOOK_OWNER = {
  "gitnexus-northstar-anchor.mjs": "northstars",
  "gitnexus-context-pressure.mjs": "taskcore",
  "gitnexus-precompact.mjs": "taskcore",
  "gitnexus-grep-guard.mjs": "gitnexus",
  "gitnexus-read-guard.mjs": "gitnexus",
  "gitnexus-edit-guard.mjs": "gitnexus",
  "gitnexus-bash-guard.mjs": "gitnexus",
  "gitnexus-mcp-guard.mjs": "gitnexus",
  // gitnexus-session.mjs is CORE: it carries the north-stars pointer and session hygiene, and
...
/** @returns {Set<string>} every .gnkit/lib basename reachable from a core entry point */
...
/** .gnkit/lib modules that only make sense with the graph (minus anything core needs). */
...
export function featureOf(rel) {
  ...
  if (rel.startsWith(".gnkit/lib/")) {
    const base = rel.slice(".gnkit/lib/".length);
  ...
  for (const [dir, id] of Object.entries(SKILL_OWNER)) {
    if (rel.startsWith(`skills/${dir}/`) || rel.startsWith(`.gnkit/skills/${dir}/`)) return id;
  }
  if (rel.startsWith("skills/") || rel.startsWith(".gnkit/skills/")) return "gitnexus";
