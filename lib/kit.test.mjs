...
describe("bearing", () => {
  ...
  it("language packs: every pack is OWNED, and the owner gates it", async () => {
    const { featureOf, FEATURE_IDS, LANG_PACK_OWNER, parseFeatures } = await import(
      new URL("./features.mjs", import.meta.url).href
    );
    const { shouldCopyBundleFile } = await import(
      new URL("./kit-shared.mjs", import.meta.url).href
    ...
    // a TypeScript rulebook installed silently into every Python repo. Listing the packs in the
    // test would go stale with the map it is meant to police (GP-11).
    const packDir = path.join(BUNDLE_ROOT, ".bearing/lang");
    const packs = fs.readdirSync(packDir).filter((f) => f.endsWith(".md"));
    assert.ok(packs.length, "no language packs found — the probe is wrong, not the bundle");
    for (const f of packs) {
      const owner = LANG_PACK_OWNER[f];
      assert.ok(owner, `.bearing/lang/${f} has no owner — it would ship to EVERY install`);
      assert.ok(
        FEATURE_IDS.includes(owner),
        `.bearing/lang/${f} is owned by "${owner}", which is not an installable feature`,
      );
      assert.equal(featureOf(`.bearing/lang/${f}`), owner);
      // The feature axis must actually gate it, in both directions.
      const other = FEATURE_IDS.find((id) => id !== owner);
      assert.equal(
        shouldCopyBundleFile(`.bearing/lang/${f}`, "claude", parseFeatures(owner)),
        true,
      );
      assert.equal(
        shouldCopyBundleFile(`.bearing/lang/${f}`, "claude", parseFeatures(other)),
        false,
      );
    }
