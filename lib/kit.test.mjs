...
describe("bearing", () => {
  ...
  it("the task-core is checkpointed through the session, not only at the top", async () => {
    // Fullness alone is a poor trigger, and fixing the window made that visible: 90% of a correct 1M
    // window is 900,000 tokens, which 6 of 404 real sessions on one machine ever reached. The single
    // prompt to save state fired in 1.5% of sessions — it only LOOKED like it worked because the
    // false alarm fired early off a wrong window.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-band-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"b"}\n');
    ...
    fs.writeFileSync(
      path.join(tmp, ".bearing", "hooks.local.json"),
      JSON.stringify({ contextWindowTokens: 1_000_000 }),
    );
    ...
    const at = (tok) => {
      fs.writeFileSync(
        jsonl,
        JSON.stringify({ type: "assistant", message: { usage: { input_tokens: tok } } }) + "\n",
      );
      const r = spawnSync(
        process.execPath,
        [path.join(tmp, ".claude/hooks/bearing-context-pressure.mjs")],
        {
          input: JSON.stringify({ transcript_path: jsonl, cwd: tmp }),
          encoding: "utf8",
          env: { ...process.env, CLAUDE_PROJECT_DIR: tmp },
          timeout: 20000,
        },
    ...
    assert.equal(at(50_000), "", "5% of the window is not a checkpoint");
    assert.match(at(130_000), /checkpoint/, "the first 10% band never fired");
    assert.equal(at(131_000), "", "the same band nudged twice");
    assert.match(at(240_000), /checkpoint/, "the next band never fired");
    // Skipping bands must not queue them up — one nudge for where you ARE, not one per band passed.
    assert.match(at(700_000), /checkpoint/);
    assert.equal(at(701_000), "");
    // The top band keeps its urgency, and says compaction rather than "checkpoint".
    assert.match(at(930_000), /auto-compaction is NEAR/);
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("checkpoints are not all spent before the window is known", async () => {
    // The bands divide the WINDOW, so they inherit whatever it is — and the evidence lookup only ran
    // at the 90% warning. On a 1M session with nothing to go on, all nine checkpoints landed inside
    // the first 195k tokens (announced as 13%, 30%, 53%, 80%, 98%) and then the session went silent
    // for the remaining 800k. Front-loaded and then absent is worse than the single trigger it
    // replaced.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-burn-"));
    ...
    const at = (tok) => {
      ...
      const r = spawnSync(
        ...
        [path.join(tmp, ".claude/hooks/bearing-context-pressure.mjs")],
        {
          input: JSON.stringify({ transcript_path: jsonl, cwd: tmp }),
          encoding: "utf8",
          // No history in this HOME → the genuinely-unknowable case.
          env: { ...process.env, CLAUDE_PROJECT_DIR: tmp, HOME: home },
          timeout: 20000,
        },
      );
      try {
    ...
    // Climb through the assumed 200k. These early ones are hedged, not wrong-and-confident.
    at(25_000);
    at(105_000);
    at(195_000);
    // Past 200k the window is disproven and revised to 1M. Bands spent against the old window must
    // not lock out the rest of the session.
    at(260_000);
    const rest = [at(420_000), at(610_000), at(810_000)].filter(Boolean);
    assert.equal(rest.length, 3, "the session went silent after the window was corrected");
    assert.match(rest[1], /~61%/, "the band is measured against the corrected window");
    assert.match(at(910_000), /auto-compaction is NEAR/, "the real warning never arrived");

    // And on a machine that CAN know, it must never go wrong in the first place. The lookup has to
    // run from the first band, not from the 90% warning — otherwise the early bands are measured
    // against the assumed window and 105k of a 1M session announces itself as 53% full.
    const home2 = path.join(tmp, "home2");
    const proj = path.join(home2, ".claude", "projects", "-p");
    fs.mkdirSync(proj, { recursive: true });
    fs.writeFileSync(
      path.join(proj, "past.jsonl"),
      JSON.stringify({ compactMetadata: { trigger: "auto", preTokens: 1000070 } }) + "\n",
    );
    const tmp2 = fs.mkdtempSync(path.join(os.tmpdir(), "gn-burn2-"));
    execSync("git init -q", { cwd: tmp2 });
    fs.writeFileSync(path.join(tmp2, "package.json"), '{"name":"b"}\n');
    execSync("git add -A && git commit -qm i", { cwd: tmp2, shell: "/bin/bash" });
    installKit(tmp2, { runtime: "claude", features: "northstars,taskcore", runSetup: false, skipVerify: true });
    const j2 = path.join(tmp2, "t.jsonl");
    fs.writeFileSync(
      j2,
      JSON.stringify({ type: "assistant", message: { usage: { input_tokens: 105_000 } } }) + "\n",
    );
    const r2 = spawnSync(
      process.execPath,
      [path.join(tmp2, ".claude/hooks/bearing-context-pressure.mjs")],
      {
        input: JSON.stringify({ transcript_path: j2, cwd: tmp2 }),
        encoding: "utf8",
        env: { ...process.env, CLAUDE_PROJECT_DIR: tmp2, HOME: home2 },
        timeout: 20000,
      },
    );
    const msg = (() => {
      try {
        return JSON.parse(r2.stdout).hookSpecificOutput.additionalContext;
      } catch {
        return "";
      }
    })();
    assert.match(msg, /~11%/, `105k of a known 1M window is 11%, not what it said: ${msg.slice(0, 40)}`);
