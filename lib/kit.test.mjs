...
describe("gitnexus-agent-kit", () => {
  ...
  it("migration removes legacy hook files, but only when superseded", async () => {
    ...
    fs.writeFileSync(path.join(hooks, "bearing-grep-guard.mjs"), "new");
    ...
    assert.ok(!fs.existsSync(path.join(hooks, "gitnexus-grep-guard.mjs")), "superseded → removed");
    assert.ok(fs.existsSync(path.join(hooks, "bearing-grep-guard.mjs")), "replacement kept");
    assert.ok(
