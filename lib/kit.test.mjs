...
describe("bearing", () => {
  ...
  it("the window correction reaches the band where it cries wolf", async () => {
    ...
    try {
      ...
      const learned = cp.contextPressure(band, { contextPressureThreshold: 0.9 });
    ...
    const proven = path.join(tmp, "proven.jsonl");
    fs.writeFileSync(
      proven,
    ...
    // and a set window is treated as the user's own statement of fact, so the estimator returned it
    // and stopped. Every correction was unreachable in production while its unit test passed by
    ...
    try {
      ...
      assert.equal(viaRealConfig.over, false, "the false alarm is back through the real config path");
      fs.rmSync(repo, { recursive: true, force: true });
    } finally {
    ...
    assert.equal(
      cp.contextPressure(full, { contextWindowTokens: 200_000, contextPressureThreshold: 0.9 }).over,
      true,
