...
describe("gitnexus-agent-kit", () => {
  ...
  it("runtime filter skips cursor paths for zed-only", () => {
    ...
    assert.equal(
      shouldCopyBundleFile(".bearing/gitnexus-hooks.json", "zed"),
      true,
    );
    assert.equal(
  ...
  it("bundle includes agent reasoning shortcuts", () => {
    ...
    assert.ok(files.includes(".bearing/gitnexus-hooks.json"));
  ...
  it("hook config enforces polyglot source extensions", async () => {
    ...
    fs.writeFileSync(
      path.join(tmp, ".bearing/gitnexus-hooks.json"),
  ...
  it("GITNEXUS_CONTEXT_WINDOW env overrides the shared config file (per-machine window)", async () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(repo, ".bearing/gitnexus-hooks.json"),
  ...
  it("gitnexus-hooks.local.json overrides the shared file per-machine (defaults < shared < local < env)", async () => {
    ...
    try {
      ...
      const shared = path.join(r, ".bearing/gitnexus-hooks.json");
      const local = path.join(r, ".bearing/gitnexus-hooks.local.json");
  ...
  it("intel-only install: no GitNexus files, and every installed hook still RUNS", () => {
    ...
    fs.writeFileSync(
      path.join(tmp, ".bearing/gitnexus-northstars.md"),
      "# NS\n- **NS-1** — Test invariant.\n",
    );
  ...
  it("rename migration: .gnkit -> .bearing PRESERVES the user's irreplaceable files", async () => {
    ...
    fs.writeFileSync(path.join(tmp, ".gnkit/.gitnexus-task-core.md"), "in-flight task state");
    fs.writeFileSync(
    ...
    assert.match(
      fs.readFileSync(path.join(tmp, ".bearing/gitnexus-northstars.md"), "utf8"),
      /Irreplaceable invariant/,
      "north-stars survived",
    );
    assert.match(
      fs.readFileSync(path.join(tmp, ".bearing/.gitnexus-task-core.md"), "utf8"),
      /in-flight/,
      "task-core survived",
    );
    assert.match(
      fs.readFileSync(path.join(tmp, ".bearing/gitnexus-hooks.local.json"), "utf8"),
      /1000000/,
