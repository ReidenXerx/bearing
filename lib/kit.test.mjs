...
describe("bearing", () => {
  ...
  it("a module that no active runtime supports is skipped, loudly (NS-14)", async () => {
    ...
    const rule = fs.readFileSync(
      path.join(cursorOnly, ".cursor/rules/00-bearing-enforcement.mdc"),
      "utf8",
    );
  ...
  it("`bearing update` with no path means this repo, and says so when it cannot", () => {
    ...
    const cli = path.join(path.dirname(new URL(import.meta.url).pathname), "kit.mjs");
    const run = (cwd, args) =>
  ...
  it("no skill advertises a directory migration deletes", () => {
    ...
    for (const name of fs.readdirSync(dir)) {
      ...
      assert.ok(
        ...
        `${name} points at .cursor/skills/generated, which migration removes`,
      );
    }
  });
  ...
  it("resource URIs use the scheme the MCP server actually serves", () => {
    ...
    const files = [new URL("../scripts/contract/enforcement-contract.md", import.meta.url)];
    const dir = new URL("../bundle/skills/", import.meta.url);
    for (const name of fs.readdirSync(dir)) {
      const f = new URL(`${name}/SKILL.md`, dir);
    ...
    for (const f of files) {
      ...
      assert.ok(
        ...
        `${f.pathname.split("/").pop()} teaches bearing:// — the server serves gitnexus://`,
      );
    }
  });
  ...
  it("no scorecard counter can be written and never shown", () => {
    // The scorecard rendered `Object.keys(labels).filter(...)` — a hand-kept label map on the READ
    // side of a counter, which is the same drift as a hand-kept list on the write side. Four
    // counters were bumped on every event and displayed to nobody, two of them for months
    // (`northStarAnchors`, `impactVerdictsQuestioned`) before two more joined them the same day.
    const agent = fs.readFileSync(path.join(BUNDLE_ROOT, "scripts/bearing-agent.mjs"), "utf8");
    const block = agent.slice(agent.indexOf('cmd === "scorecard"'), agent.indexOf('cmd === "stats"'));
    assert.ok(
      !/Object\.keys\(labels\)/.test(block),
      "the scorecard still filters by the label map, so an unlabelled counter stays invisible",
    );
    assert.match(block, /Object\.keys\(counts\)/, "the scorecard should render what was counted");
    assert.match(block, /labels\[k\] \?\?/, "no fallback label for a counter nobody named");
  ...
  it("the Cursor hooks fail open too — same class, different runtime", () => {
    ...
    const hooks = fs.readdirSync(path.join(tmp, ".cursor/hooks")).filter((f) => f.endsWith(".sh"));
  ...
  it("every runtime the fallback message NAMES gets a command that installs it", () => {
    ...
    const src = fs.readFileSync(path.join(path.dirname(new URL(import.meta.url).pathname), "kit.mjs"), "utf8");
