...
describe("bearing", () => {
  ...
  it("`statusline` installs user-wide, and never over one the user wrote", async () => {
    ...
    try {
      ...
      // 4. THEIRS is never touched — the case this whole design exists for.
      const mine = { type: "command", command: "~/.claude/statusline.py", refreshInterval: 15 };
      fs.writeFileSync(settings, `${JSON.stringify({ statusLine: mine, env: { MINE: "1" } }, null, 2)}\n`);
      const before = fs.readFileSync(settings, "utf8");
      installStatusLine();
      assert.equal(fs.readFileSync(settings, "utf8"), before, "overwrote a status line we did not write");
      assert.ok(fs.existsSync(script), "should still install the script for them to point at");
      installStatusLine({ remove: true });
      assert.deepEqual(
        JSON.parse(fs.readFileSync(settings, "utf8")).statusLine,
        mine,
        "removal took a status line we did not write",
      );
    } finally {
      process.env.HOME = origHome;
    }
