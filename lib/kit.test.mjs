...
describe("bearing", () => {
  ...
  it("every deny names its exit, including the ones that never did (NS-6)", async () => {
    // Six of the seven deny branches named no exit, and `mode: guide` was reachable from none of
    // them by default: `ESCAPE_HINT` is appended only to the must_refresh message, which cannot
    // fire while `stalenessGate` defaults to "off". So the documented way out of a wrong gate was,
    // in the shipped configuration, unreachable from the gates themselves. Asserted at the emit
    // choke point rather than per branch, so a NEW deny cannot ship without one.
    const tmp = setupKitRepo({ fresh: true });
    const denied = runHook(tmp, "bearing-edit-guard.mjs", {
      tool_name: "Edit",
      tool_input: { file_path: path.join(tmp, "src/app.ts"), old_string: "a", new_string: "b" },
    });
    assert.equal(denied.permission, "deny");
    assert.match(
      denied.agent_message,
      /bearing-agent\.mjs fallback|bearing:fallback/,
      "a deny that does not name the escape hatch",
    );
    assert.match(denied.agent_message, /mode": ?"guide|mode: ?guide/, "no permanent downgrade named");
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("a guarded reference is recognised, and a fake guard is not", async () => {
    // `guardsPath` exists so a `[[ -f X ]]`-guarded call is not counted as dangling. Its first
    // version was fooled five ways and false-alarmed two more, which is worse than not having it:
    // a check that both misses real danglers AND fails correct installs.
    const src = fs.readFileSync(new URL("./postcheck.mjs", import.meta.url), "utf8");
    const fn = new Function(`${src.match(/function guardsPath[\s\S]*?\n}/)[0]}; return guardsPath;`)();
    const T = "scripts/ghost.mjs";
    for (const [body, want, why] of [
      ["if [[ -f scripts/ghost.mjs ]]; then node scripts/ghost.mjs; fi", true, "same-branch guard"],
      ["[[ ! -f scripts/ghost.mjs ]] || node scripts/ghost.mjs", true, "negative guard"],
      ['[[ -f "$ROOT/scripts/ghost.mjs" ]] && node scripts/ghost.mjs', true, "$ROOT prefix"],
      ['if (existsSync("scripts/ghost.mjs")) run()', true, "existsSync"],
      ["# [[ -f scripts/ghost.mjs ]] || exit 0\nnode scripts/ghost.mjs", false, "guard in a comment"],
      ["[[ -f scripts/ghost.mjs.bak ]] && echo\nnode scripts/ghost.mjs", false, "guard on a longer path"],
      ["node scripts/ghost.mjs", false, "no guard at all"],
    ]) {
      assert.equal(fn(body, T), want, why);
    }
  });

  it("hook commands carry no shell expansion, so they work in any shell (#18)", async () => {
    // Claude Code runs a hook through whichever shell it finds — Git Bash when present, PowerShell
    // otherwise — and `$CLAUDE_PROJECT_DIR` only expands under a POSIX one. On Windows all 13 hooks
    // silently failed to launch, and hook errors are non-blocking, so with `"mode": "enforce"` the
    // guards were absent while appearing installed. There is no spelling that works in both shells:
    // `$env:CLAUDE_PROJECT_DIR` fixes PowerShell and breaks bash. So the command must not contain a
    // variable at all — the scripts resolve the root themselves.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-shellfree-"));
    execSync("git init -q", { cwd: tmp });
    execSync("git config user.email t@t.com && git config user.name t", { cwd: tmp, shell: true });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"w"}\n');
    execSync("git add -A && git commit -qm i", { cwd: tmp, shell: true });
    ...
    assert.ok(commands.length >= 5, "no hook commands to check");
    for (const cmd of commands) {
      assert.ok(!cmd.includes("$"), `hook command contains a shell expansion: ${cmd}`);
      // The INVOKER, not the whole string — one of the guards is called `bearing-bash-guard.mjs`.
      assert.match(cmd, /^node /, `hook command does not invoke node directly: ${cmd}`);
      assert.ok(!/%\w+%/.test(cmd), `hook command uses cmd.exe expansion: ${cmd}`);
    }

    // And the script must resolve the root on its own, with the variable ABSENT — the PowerShell
    // case, where it expands to nothing.
    const env = { ...process.env };
    delete env.CLAUDE_PROJECT_DIR;
    const r = spawnSync(process.execPath, [".claude/hooks/bearing-grep-guard.mjs"], {
      cwd: tmp,
      env,
      input: JSON.stringify({
        tool_name: "Grep",
        tool_input: { pattern: "handlePayment" },
        cwd: tmp,
      }),
      encoding: "utf8",
    });
    assert.equal(r.status, 0, `the guard crashed without CLAUDE_PROJECT_DIR:\n${r.stderr}`);

    // AND THE COMMAND MUST ACTUALLY RUN, from a cwd that is not the project root.
    //
    // The first fix for #18 was a bare relative path, justified by "Claude Code runs hooks with cwd
    // at the project root". That premise is false, and this test did not catch it because it only
    // ever INSPECTED the string — a command can contain no shell variable, start with `node`, and
    // still be unable to find its own entry file. The result was a MODULE_NOT_FOUND loader trace on
    // every tool call in a real repo, silent because hook errors are non-blocking (NS-10, NS-12).
    const sub = path.join(tmp, "src", "deep");
    fs.mkdirSync(sub, { recursive: true });
    const oneCmd = commands.find((c) => c.includes("bearing-grep-guard.mjs"));
    assert.ok(oneCmd, "no grep-guard command to execute");
    const payload = JSON.stringify({
      tool_name: "Grep",
      tool_input: { pattern: "handlePayment" },
      cwd: tmp,
    });
    for (const [label, cwd, projectDir] of [
      ["project root", tmp, tmp],
      ["subdirectory", sub, tmp],
    ]) {
      const run = spawnSync(oneCmd, {
        cwd,
        shell: true,
      ...
      assert.equal(
        run.status,
        0,
        `the hook command failed when run from the ${label}:\n${oneCmd}\n${run.stderr}`,
      );
      assert.doesNotMatch(
        run.stderr ?? "",
        /MODULE_NOT_FOUND|Cannot find module/,
        `the hook command could not locate its entry file from the ${label}`,
      );
    }
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("bearing's own stanzas are taken back out of Claude Code's memory index (#19)", async () => {
    // The write is gone; this removes what it left. Deliberately narrow: only OUR marker followed
    // by lines we wrote. A stanza the user annotated keeps its marker but not our shape and stays,
    // and so does every real pointer line — the whole point of the index.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-memidx-"));
    execSync("git init -q", { cwd: tmp });
    execSync("git config user.email t@t.com && git config user.name t", { cwd: tmp, shell: true });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"m"}\n');
    execSync("git add -A && git commit -qm i", { cwd: tmp, shell: true });

    const origHome = process.env.HOME;
    process.env.HOME = tmp;
    try {
      const memDir = path.join(tmp, ".claude/projects", path.resolve(tmp).replace(/\//g, "-"), "memory");
      ...
      fs.writeFileSync(
        ...
        `# Project working memory (bearing)\n\n` +
          ...
          `\n<!-- checkpoint 2026-08-13T14:40:30.764Z -->\n` +
          `- trigger: manual | index: fresh | gates: impact done, detect_changes done\n` +
          `- (transcript about to be summarized — task/decisions/open-items/file:line above must already be current)\n` +
          `\n- [Toolchain](toolchain.md) — the one real entry\n` +
          `\n<!-- checkpoint 2026-09-01T15:59:05.455Z -->\n` +
          `- trigger: auto | index: fresh | gates: impact pending, detect_changes done\n` +
          `- (transcript about to be summarized — task/decisions/open-items/file:line above must already be current)\n` +
          `\n<!-- checkpoint 2026-09-02T00:00:00.000Z -->\n` +
          `- trigger: manual — I ANNOTATED THIS ONE, keep it\n`,
      );
      installKit(tmp, { runtime: "claude", features: "all", runSetup: false, skipVerify: true });
      const after = fs.readFileSync(mem, "utf8");

      assert.match(after, /- \[Toolchain\]\(toolchain\.md\)/, "removed a real index entry");
      assert.ok(!/gates: impact done/.test(after), "left one of bearing's own stanzas behind");
      assert.ok(!/Project working memory \(bearing\)/.test(after), "left the misdescribing header");
      assert.match(after, /I ANNOTATED THIS ONE/, "removed a stanza the user had edited");
    } finally {
  ...
  it("a repo with NO index is not enforced against a graph that does not exist", async () => {
    // `check-staleness` reports `missing`, and the gate-off branch mapped that to phase "fresh" —
    // so full enforcement fired and every redirect named a repo the MCP server has never heard of.
    // Reachable three ways, none exotic: a linked git worktree, a corrupt meta.json, and every repo
    // between install and its first index (`--skip-verify` skips that build; `dogfood` uses it).
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-nograph-"));
    for (const reason of ["missing", "invalid_meta", "not_git"]) {
      const policy = evaluateStalePolicy({ fresh: false, reason }, tmp);
      assert.equal(policy.phase, "classical_fallback", `${reason} still enforced`);
      assert.equal(policy.allowClassical, true, `${reason} did not allow classical tools`);
      assert.match(
        staleRefreshAgentMessage({ fresh: false, reason }, policy),
        /NO GRAPH HERE/,
        `${reason} did not say the graph is absent`,
      );
    }
    // NEGATIVE — an index that merely LAGS is a different thing and must not take this branch.
    assert.notEqual(
      evaluateStalePolicy({ fresh: false, reason: "behind", detail: "5 behind" }, tmp).phase,
      "classical_fallback",
      "a merely stale index was treated as no index at all",
    );
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("a wedged git cannot hang the hook path (NS-7)", () => {
    // Nothing here was bounded: `claude-emit` spawns `load-staleness`, which spawns
    // `check-staleness`, which shells out to git several times. A git that never returns left the
    // Grep guard blocked with no verdict, and because the cache was written only on SUCCESS every
    // subsequent tool call re-paid it. Measured with a PATH-shimmed `git` that sleeps: unbounded
    // before, ~6s worst case now — one bound per hook invocation, not one per git call.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-wedged-"));
    execSync("git init -q", { cwd: tmp });
    execSync("git config user.email t@t.com && git config user.name t", { cwd: tmp, shell: true });
    ...
    fs.writeFileSync(
      path.join(tmp, ".gitnexus/meta.json"),
      JSON.stringify({ lastCommit: head, stats: { nodes: 10, embeddings: 10 } }),
    );
    ...
    const r = spawnSync(
      process.execPath,
      [path.join(BUNDLE_ROOT, ".bearing/lib/check-staleness.mjs"), tmp],
      { encoding: "utf8", env: { ...process.env, PATH: `${bin}:${process.env.PATH}` } },
    );
    const elapsed = Date.now() - started;
    assert.ok(elapsed < 30000, `a wedged git blocked the hook path for ${elapsed}ms`);
    assert.ok(r.stdout?.trim(), "no verdict at all — the hook had nothing to emit");
    JSON.parse(r.stdout.trim()); // and it is still well-formed
    fs.rmSync(tmp, { recursive: true, force: true });
  });
  ...
  it("installKit claude runtime wires MCP, hooks, CLAUDE.md, skills", () => {
    ...
    // NO STATUS LINE from a repo install — it is machine-wide, and `bearing statusline` is the
    // one-time opt-in that owns it. Covered properly by its own tests below.
    assert.equal(settings.statusLine, undefined, "a repo install wired a machine-wide setting");
    const m = readManifest(tmp);
    assert.equal(m?.data.runtime, "claude");
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("the shipped status line renders a status payload without erroring", () => {
    // The script is shell, so nothing else in this suite would catch a syntax error or
    // a stray write to stdout — it would simply render as a broken line for every user.
    // templates/ rather than .claude/: the script is installed USER-WIDE by
    // `bearing statusline`, never copied into a repo, and templates/ is the one bundle
    // directory `shouldCopyBundleFile` deliberately never copies.
    const script = path.join(BUNDLE_ROOT, "templates/statusline.sh");
    const payload = JSON.stringify({
      ...
      rate_limits: {
        five_hour: { used_percentage: 20, resets_at: 0 },
        seven_day: { used_percentage: 5, resets_at: 0 },
      },
    });
    const r = spawnSync("bash", [script], { input: payload, encoding: "utf8" });
    assert.equal(r.status, 0, `status line exited ${r.status}: ${r.stderr}`);
    assert.equal(r.stderr, "", "status line writes nothing to stderr");
    assert.match(r.stdout, /Test Model/, "renders the model name from the payload");
    // Vitals live on their own row, and switching them off has to take the whole row
    // with it rather than leave a blank line behind. Asserted in the suppressible
    // direction: the vitals themselves depend on the host — CI has no discrete GPU and
    // may have no PSI — so "always three rows" would be flaky by design.
    const off = spawnSync("bash", [script], {
      input: payload,
      encoding: "utf8",
      env: { ...process.env, SL_VITALS: "0" },
    });
    assert.equal(off.stdout.split("\n").length, 2, "SL_VITALS=0 leaves two rows");
    assert.ok(
      r.stdout.split("\n").length <= 3,
      "vitals add at most one row, never more",
    );
    // Garbage in must not produce a blank line — it degrades to a fixed string.
    const bad = spawnSync("bash", [script], { input: "not json", encoding: "utf8" });
    assert.equal(bad.status, 0, "malformed payload still exits 0");
    assert.ok(bad.stdout.trim().length > 0, "malformed payload still renders something");
  });

  it("every verb the CLI dispatches is routed by the published binary", () => {
    // `bin/cli.mjs` keeps its own VERBS set, and a verb missing from it does NOT become "unknown
    // command": the wrapper treats the first argument as a PATH and quietly runs an install against
    // it. `bearing statusline` became `install ./statusline` and died with "Not a git repository".
    // Invisible when you drive lib/kit.mjs directly, which is what the rest of this suite does —
    // it only shows up through the packed binary (NS-10, NS-21).
    //
    // DERIVED from the dispatcher rather than listed here, so a new verb cannot ship unrouted.
    const kit = fs.readFileSync(new URL("./kit.mjs", import.meta.url), "utf8");
    const cli = fs.readFileSync(new URL("../bin/cli.mjs", import.meta.url), "utf8");
    const dispatched = new Set(
      [...kit.matchAll(/cmd === "([\w-]+)"/g)].map((m) => m[1]).filter((v) => !v.startsWith("-")),
    );
    const routed = new Set(
      (cli.match(/const VERBS = new Set\(\[([^\]]*)\]\)/)?.[1] ?? "")
        .split(",")
        .map((t) => t.trim().replace(/^["']|["']$/g, ""))
        .filter(Boolean),
    );
    assert.ok(dispatched.size >= 4, "found no dispatched verbs — the scan is looking in the wrong place");
    const unrouted = [...dispatched].filter((v) => !routed.has(v));
    assert.deepEqual(
      unrouted,
      [],
      `cliMain dispatches these but bin/cli.mjs does not route them, so each is silently treated as an install target: ${unrouted.join(", ")}`,
    );
  });

  it("declining gold practices keeps the project's own PP-# half (NS-1)", () => {
    // `.bearing/gold-practices.md` has two owners: bearing's block above the END marker, and the
    // project's own `PP-#` rules below it, which bearing never writes. Splitting the module out of
    // `northstars` made declining it a reachable, reasonable choice — "I do not want your generic
    // rules" — and the sweep read that as permission to delete the file, taking their half with it.
    // The file is gitignored, so there was no backup and nothing in git to recover from.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-pp-"));
    execSync("git init -q", { cwd: tmp });
    execSync("git config user.email t@t.com && git config user.name t", { cwd: tmp, shell: true });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"pp"}\n');
    execSync("git add -A && git commit -qm i", { cwd: tmp, shell: true });
    installKit(tmp, {
      runtime: "claude",
      features: "northstars,goldpractices,taskcore",
      runSetup: false,
      skipVerify: true,
    });
    const gp = path.join(tmp, ".bearing/gold-practices.md");
    assert.ok(fs.existsSync(gp), "the module did not install its file");
    fs.appendFileSync(gp, "\n- **PP-1** — a rule this project earned itself.\n");

    updateKit(tmp, { features: "northstars,taskcore", runSetup: false, skipVerify: true });
    assert.ok(!fs.existsSync(gp), "a declined module left its active file in place");
    const kept = `${gp}.bearing-backup`;
    assert.ok(fs.existsSync(kept), "their PP-# half was destroyed with bearing's");
    assert.match(fs.readFileSync(kept, "utf8"), /PP-1/);

    // NEGATIVE — with nothing of theirs in it, the file is just removed and leaves no litter.
    const clean = fs.mkdtempSync(path.join(os.tmpdir(), "gn-pp-clean-"));
    execSync("git init -q", { cwd: clean });
    execSync("git config user.email t@t.com && git config user.name t", { cwd: clean, shell: true });
    fs.writeFileSync(path.join(clean, "package.json"), '{"name":"pp2"}\n');
    execSync("git add -A && git commit -qm i", { cwd: clean, shell: true });
    installKit(clean, {
      runtime: "claude",
      features: "northstars,goldpractices,taskcore",
      runSetup: false,
    ...
    updateKit(clean, { features: "northstars,taskcore", runSetup: false, skipVerify: true });
    assert.ok(!fs.existsSync(path.join(clean, ".bearing/gold-practices.md")));
    assert.ok(
      !fs.existsSync(path.join(clean, ".bearing/gold-practices.md.bearing-backup")),
      "left a backup of a file with nothing of the user's in it",
    );
    fs.rmSync(tmp, { recursive: true, force: true });
    fs.rmSync(clean, { recursive: true, force: true });
  });

  it("a language pack is opt-in, so pressing Enter does not put TS rules in a Python repo", async () => {
    // `recommended` is what the interactive default installs. A language or framework pack is by
    // definition not universal, so defaulting it on puts ~1,400 tokens of TypeScript guidance in
    // the always-on contract of every repo that cannot use it — contradicting the packs' own
    // per-language design. Same call e2e already makes.
    const { FEATURES, defaultFeatureIds } = await import(new URL("./features.mjs", import.meta.url).href);
    const byId = Object.fromEntries(FEATURES.map((f) => [f.id, f]));
    const def = new Set(defaultFeatureIds());
    for (const id of ["tsjs", "frontend", "react"]) {
      assert.equal(byId[id]?.recommended, false, `${id} is a language pack and must be opt-in`);
      assert.ok(!def.has(id), `${id} is in the default set`);
    }
    // ...and the one that was split OUT of an existing module stays on, or upgrading silently
    // drops something every installed repo already had.
    assert.equal(byId.goldpractices?.recommended, true);
    assert.ok(def.has("goldpractices"));
  });

  it("a repo install never writes a status line — it is a machine-wide setting", () => {
    // A status line is ONE bar, at the bottom of every session, in every project. Wiring it from a
    // per-repo install was wrong twice over: project settings sit above user settings in the
    // precedence chain, so installing bearing in one repo would silently replace whatever the user
    // had built for themselves — and the guard meant to prevent that read `statusLine` from the
    // PROJECT settings, which cannot see a user-level one, so it would never have fired.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-sl-repo-"));
    execSync("git init -q", { cwd: tmp });
    execSync("git config user.email t@t.com && git config user.name t", { cwd: tmp, shell: true });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"sl"}\n');
    execSync("git add -A && git commit -qm i", { cwd: tmp, shell: true });
    installKit(tmp, { runtime: "claude", features: "all", runSetup: false, skipVerify: true });

    const cfg = JSON.parse(fs.readFileSync(path.join(tmp, ".claude/settings.json"), "utf8"));
    assert.equal(cfg.statusLine, undefined, "a repo install wired a machine-wide status line");
    assert.ok(
      !fs.existsSync(path.join(tmp, ".claude/statusline.sh")),
      "the script was copied into the repo, where it has no reason to be",
    );
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("`statusline` installs user-wide, replacing theirs but keeping a way back", async () => {
    const { installStatusLine } = await import(new URL("./kit.mjs", import.meta.url).href);
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "gn-sl-home-"));
    const settings = path.join(home, ".claude/settings.json");
    const script = path.join(home, ".claude/statusline.sh");
    const origHome = process.env.HOME;
    process.env.HOME = home;
    try {
      // 1. Nothing there yet → installed and wired, with NO shell expansion in the command (#18).
      installStatusLine();
      let cfg = JSON.parse(fs.readFileSync(settings, "utf8"));
      assert.match(cfg.statusLine.command, /statusline\.sh/);
      assert.ok(!cfg.statusLine.command.includes("$"), "command carries a shell expansion");
      assert.ok(fs.existsSync(script), "script not installed");
      assert.ok((fs.statSync(script).mode & 0o111) !== 0, "script is not executable");

      // 2. Twice converges (NS-3).
      installStatusLine();
      cfg = JSON.parse(fs.readFileSync(settings, "utf8"));
      assert.equal(Object.keys(cfg).filter((k) => k === "statusLine").length, 1);

      // 3. Removal takes only ours.
      installStatusLine({ remove: true });
      assert.equal(JSON.parse(fs.readFileSync(settings, "utf8")).statusLine, undefined);
      assert.ok(!fs.existsSync(script), "script left behind");

      // 4. THEIRS is REPLACED — but never without a way back, which is what earns the replacement.
      // An earlier build refused and printed the line to paste, which is the safe-LOOKING choice and
      // the wrong one: `bearing statusline` is an explicit request, and answering it with homework
      // means the feature does not happen. Overwriting is destructive and invisible, so the answer
      // is reversibility, not refusal (NS-6).
      const mine = { type: "command", command: "~/.claude/statusline.py", refreshInterval: 15 };
      fs.writeFileSync(
        settings,
        `${JSON.stringify({ statusLine: mine, env: { MINE: "1" } }, null, 2)}\n`,
      );
      fs.writeFileSync(path.join(home, ".claude/statusline.py"), "#!/usr/bin/env python3\n");
      installStatusLine();
      let after = JSON.parse(fs.readFileSync(settings, "utf8"));
      assert.match(after.statusLine.command, /statusline\.sh/, "did not replace theirs");
      // Their OPTIONS survive: only the command is ours to change.
      assert.equal(after.statusLine.refreshInterval, 15, "dropped an option they had set");
      assert.deepEqual(after.env, { MINE: "1" }, "clobbered an unrelated setting");
      // Their SCRIPT is never touched — only the settings entry moved.
      assert.ok(
        fs.existsSync(path.join(home, ".claude/statusline.py")),
        "deleted the user's own status line script",
      );
      // The displaced config is written out verbatim, at a path the warning names.
      const backup = path.join(home, ".claude/statusline.replaced.json");
      assert.ok(fs.existsSync(backup), "replaced a status line without saving it");
      assert.deepEqual(JSON.parse(fs.readFileSync(backup, "utf8")), mine, "saved the wrong thing");

      // 5. Removal PUTS THEIRS BACK. Leaving them with no status line at all is not "as we found
      // it", and it is the whole justification for replacing by default.
      installStatusLine({ remove: true });
      assert.deepEqual(
        JSON.parse(fs.readFileSync(settings, "utf8")).statusLine,
        mine,
        "removal did not restore the status line bearing replaced",
      );
      assert.ok(!fs.existsSync(backup), "the backup was left behind after being restored");

      // 6. Idempotent: replacing twice must not overwrite the backup with OUR OWN line, which would
      // make the original unrecoverable on the second run (NS-3).
      installStatusLine();
      installStatusLine();
      assert.deepEqual(
        JSON.parse(fs.readFileSync(backup, "utf8")),
        mine,
        "a second install overwrote the backup with bearing's own line",
      );
    } finally {
      process.env.HOME = origHome;
    }
  ...
  it("uninstall still takes back a status line an earlier build wired per repo", () => {
    // 1.2.0 briefly wired it into the project settings. Those repos exist, so uninstall has to be
    // able to undo it — and must leave a settings file it changed nothing in byte-identical.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-sl-legacy-"));
    execSync("git init -q", { cwd: tmp });
    execSync("git config user.email t@t.com && git config user.name t", { cwd: tmp, shell: true });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"sl2"}\n');
    execSync("git add -A && git commit -qm i", { cwd: tmp, shell: true });
    installKit(tmp, { runtime: "claude", features: "all", runSetup: false, skipVerify: true });
    const settings = path.join(tmp, ".claude/settings.json");
    const cfg = JSON.parse(fs.readFileSync(settings, "utf8"));
    cfg.statusLine = { type: "command", command: 'bash "$CLAUDE_PROJECT_DIR/.claude/statusline.sh"' };
    fs.writeFileSync(settings, JSON.stringify(cfg, null, 2));
    uninstallKit(tmp);
    const after = fs.existsSync(settings) ? JSON.parse(fs.readFileSync(settings, "utf8")) : {};
    assert.equal(after.statusLine, undefined, "left a per-repo status line behind");
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("an uninstall that removes nothing leaves the settings file byte-identical", async () => {
    // The `touched` flag was set on the mere PRESENCE of a `hooks` key, so a settings file made
    // entirely of the user's own hooks was still rewritten — and therefore reformatted — by an
    // uninstall that changed nothing in it. That is exactly what the flag was added to prevent.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-sl-untouched-"));
    fs.mkdirSync(path.join(tmp, ".claude"), { recursive: true });
    const theirs = `{\n    "hooks": {\n        "PreToolUse": [ { "matcher": "Bash", "hooks": [ { "type": "command", "command": "mine.sh" } ] } ]\n    },\n    "env": { "MINE": "1" }\n}\n`;
    const settings = path.join(tmp, ".claude/settings.json");
    fs.writeFileSync(settings, theirs);
    const { claudeAdapter } = await import(new URL("./adapters/claude.mjs", import.meta.url).href);
    claudeAdapter.unwire(tmp, {});
    assert.equal(fs.readFileSync(settings, "utf8"), theirs, "rewrote a file it changed nothing in");
    fs.rmSync(tmp, { recursive: true, force: true });
  });
  ...
  it("compaction middleware: source-aware clear + durable memory helpers", async () => {
    ...
    try {
      ...
      // bearing RESOLVES that path and never writes to it. Under the Claude Code CLI it is the
      // memory INDEX — loaded into context every session, one pointer line per memory — and the
      // PreCompact hook used to append a two-line breadcrumb to it on every compaction. Nothing
      // read them back, so the "checkpoint" only ever bought a permanent tax on the window it
      // claimed to protect. This is the negative that keeps it from coming back.
      assert.equal(
        typeof session.appendMemoryCheckpoint,
        "undefined",
        "bearing must not have a writer for Claude Code's memory index",
      );
      assert.ok(!fs.existsSync(mem), "resolving the memory path must not create the file");
    } finally {
      process.env.HOME = origHome;
    }
    fs.rmSync(tmp, { recursive: true, force: true });
  ...
  it("claude PreCompact hook emits no stdout (side-effect only) + checkpoints memory", async () => {
    ...
    const mem = session.memoryPath(tmp);
    process.env.HOME = origHome;
    // The hook COUNTS the compaction and writes nothing else. It used to append a stanza here; one
    // real project accumulated nine near-identical ones around a single genuine entry, and the
    // header it created ("Project working memory") permanently misdescribed Claude Code's index.
    assert.ok(!fs.existsSync(mem), "PreCompact wrote into Claude Code's memory index");
    const score = JSON.parse(
      fs.readFileSync(path.join(tmp, ".bearing/.gitnexus-scorecard.json"), "utf8"),
    );
    assert.ok((score.counts?.compactions ?? 0) > 0, "the compaction was not counted");
    fs.rmSync(tmp, { recursive: true, force: true });
  });
  ...
  it("north-stars: tracked path, digest parsing, anchor counter", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "",
        // The HEADING form. A real repo wrote all 15 of its north-stars this way and got a digest
        // of ZERO — the hook then took its "no NS-# lines yet" exit and emitted nothing on every
        // fire, silently, so the anchor simply did not happen there. A structure the author chose
        // is not noise; this parser has to cover the ordinary ways a person numbers a doc.
        "### NS-4 — Build from the design system, never from raw HTML",
        "#### NS-5 — Deep headings count too.",
      ].join("\n"),
    );

    assert.equal(sp.northStarsExists(tmp), true);
    const digest = sp.northStarsDigest(tmp);
    assert.equal(digest.length, 5, "only NS-# lines, prose skipped");
    ...
    assert.match(digest[3], /^NS-4 — Build from the design system/, "'### NS-4' heading form lost");
    assert.match(digest[4], /^NS-5 — Deep headings/, "'#### NS-5' heading form lost");
    // A '#' must not become a wildcard: prose that merely mentions NS-# mid-sentence stays out.
    assert.ok(
      !digest.some((l) => /Some prose/.test(l)),
      "the widened pattern started matching prose",
    );
  ...
  it("gold practices are their own module, and north-stars stops claiming them", async () => {
    const { featureOf, inheritSplitFeatures, FEATURE_IDS } = await import(
      new URL("./features.mjs", import.meta.url).href
    );
    const { filterContractByFeatures } = await import(
      new URL("./contract-filter.mjs", import.meta.url).href
    );
    assert.ok(FEATURE_IDS.includes("goldpractices"));
    assert.equal(featureOf(".bearing/gold-practices.md"), "goldpractices");

    // A recorded selection inherits what was split out of it; an unrelated one gains nothing.
    assert.deepEqual(
      [...inheritSplitFeatures(new Set(["northstars"]))].sort(),
      ["goldpractices", "northstars"],
    );
    assert.deepEqual([...inheritSplitFeatures(new Set(["taskcore"]))], ["taskcore"]);

    const md = fs.readFileSync(path.join(BUNDLE_ROOT, "templates/CLAUDE.gitnexus.md"), "utf8");
    // The whole point of the split: north-stars WITHOUT gold practices must not mention them. A
    // pointer to a file the install did not create is the NS-13 failure, in the authoritative doc.
    const nsOnly = filterContractByFeatures(md, new Set(["northstars"]));
    assert.match(nsOnly, /northstars\.md/, "north-stars lost its own section");
    assert.doesNotMatch(nsOnly, /gold-practices/, "north-stars still points at a file it no longer installs");
    assert.doesNotMatch(nsOnly, /GP-\d/);

    // Gold practices WITHOUT north-stars must still be a complete section...
    const gpOnly = filterContractByFeatures(md, new Set(["goldpractices"]));
    assert.match(gpOnly, /## Gold practices/);
    assert.match(gpOnly, /GP-1/);
    // ...minus the precedence rule, which is only meaningful when there are north-stars to outrank.
    assert.doesNotMatch(gpOnly, /outranks/);
    assert.match(
      filterContractByFeatures(md, new Set(["goldpractices", "northstars"])),
      /outranks/,
      "both installed, but the precedence rule between them went missing",
    );
  });

  it("a repo that said yes to north-stars does not lose gold practices to the split", () => {
    // The hazard is not hypothetical: update REMOVES a file whose owning feature is not selected,
    // and every 1.1.x manifest records `northstars` — an id that no longer owns this file. Without
    // inheritance, agreeing to the module in an older version is exactly what deletes it in a newer
    // one, silently, in someone else's repository (NS-1).
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-gpsplit-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"g"}\n');
    installKit(tmp, { runtime: "claude", features: "northstars", runSetup: false, skipVerify: true });

    // Reproduce the PRE-SPLIT state rather than trusting today's installer to create it: back then
    // northstars owned the file, so it was on disk and claimed by the manifest.
    const gp = path.join(tmp, ".bearing/gold-practices.md");
    fs.writeFileSync(gp, "# Gold practices — placeholder from the previous version\n");
    const mfPath = path.join(tmp, MANIFEST_PATH);
    const mf = JSON.parse(fs.readFileSync(mfPath, "utf8"));
    assert.deepEqual(mf.features, ["northstars"], "fixture does not reproduce a pre-split manifest");
    mf.files.push(".bearing/gold-practices.md");
    fs.writeFileSync(mfPath, JSON.stringify(mf, null, 2) + "\n");

    updateKit(tmp, { runSetup: false, skipVerify: true });

    assert.ok(fs.existsSync(gp), "update deleted gold practices from a repo that had opted into them");
    assert.match(
      fs.readFileSync(gp, "utf8"),
      /GP-1/,
      "the file survived but was never refreshed — it is bearing's, not seed-once",
    );
    // And the new id is recorded, so the next update needs no inheritance to do the right thing.
    assert.ok(readManifest(tmp).data.features.includes("goldpractices"));
  });

  it("an explicit --features is honoured as written, inheritance is only for what was recorded", () => {
    // Inheriting on an EXPLICIT selection would make the flag a lie: the user typed the new id set
    // with goldpractices available and left it out.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-gpexplicit-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"g"}\n');
    installKit(tmp, { runtime: "claude", features: "northstars", runSetup: false, skipVerify: true });
    assert.equal(
      ...
      false,
      "an explicit --features northstars still shipped gold practices",
    );
    assert.ok(fs.existsSync(path.join(tmp, ".bearing/skills/bearing-northstars/SKILL.md")));

    // ...and the module stands alone in the other direction too.
    const solo = fs.mkdtempSync(path.join(os.tmpdir(), "gn-gpsolo-"));
    execSync("git init -q", { cwd: solo });
    fs.writeFileSync(path.join(solo, "package.json"), '{"name":"s"}\n');
    installKit(solo, { runtime: "claude", features: "goldpractices", runSetup: false, skipVerify: true });
    assert.ok(fs.existsSync(path.join(solo, ".bearing/gold-practices.md")));
    assert.match(fs.readFileSync(path.join(solo, "CLAUDE.md"), "utf8"), /## Gold practices/);
    assert.equal(fs.existsSync(path.join(solo, ".bearing/skills/bearing-northstars/SKILL.md")), false);
  });

  it("a manifest-less clone infers EVERY module on disk, not just the ones we remembered", async () => {
    // inferInstallFromDisk exists so a teammate who clones a repo with a tracked .bearing/ can run
    // `bearing update` at all. It enumerates modules BY HAND, and update removes files whose feature
    // is not selected — so a module missing from that list is a module whose files an update
    // silently deletes from a repo that plainly has them. Two were missing the moment this branch
    // added them, and nothing failed.
    //
    // Derived from FEATURE_IDS rather than a second hand-kept list, because the hand-kept list is
    // the thing under test (GP-11).
    const { FEATURE_IDS } = await import(new URL("./features.mjs", import.meta.url).href);
    const { inferInstallFromDisk } = await import(new URL("./kit.mjs", import.meta.url).href);
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-infer-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"i"}\n');
    installKit(tmp, { runtime: "claude", features: "all", runSetup: false, skipVerify: true });

    // The fresh-clone case: the manifest is gitignored, so it is exactly what a teammate lacks.
    fs.rmSync(path.join(tmp, MANIFEST_PATH), { force: true });
    const inferred = inferInstallFromDisk(tmp).features;
    assert.deepEqual(
      FEATURE_IDS.filter((id) => !inferred.includes(id)),
      [],
      "installed and visible on disk, but not inferred — the next update deletes these",
    );
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("EVERY module is optional: declining one removes its files and nothing else", async () => {
    // "Each feature must stand alone" is the header of features.mjs, and until now it was checked
    // one module at a time, by hand — which means the guarantee held for the modules somebody
    // remembered to write a test for. This is the same claim, derived: it iterates FEATURE_IDS, so
    // a module added tomorrow is covered the day it appears (GP-11).
    const { FEATURE_IDS, featureOf } = await import(new URL("./features.mjs", import.meta.url).href);

    // Where a bundle path ENDS UP once installed. Skills are the only ones that move: the canonical
    // store is .bearing/skills/, and the per-runtime dirs are symlinks into it.
    const installedPath = (rel) => (rel.startsWith("skills/") ? `.bearing/${rel}` : rel);

    // The always-on Cursor rule is GENERATED by the adapter for every install, with its content
    // feature-filtered — it is not the bundle copy, which featureOf assigns to gitnexus so the two
    // graph playbooks beside it do not ship. Its presence therefore proves nothing about a module,
    // and its CONTENT is what matters; the contract test below checks that on both renders.
    const GENERATED = new Set([".cursor/rules/00-bearing-enforcement.mdc"]);
    const ownedBy = Object.fromEntries(
      FEATURE_IDS.map((id) => [
        id,
        listBundleFiles()
          .filter((r) => featureOf(r) === id)
          .map(installedPath)
          .filter((r) => !GENERATED.has(r)),
      ]),
    );
    for (const id of FEATURE_IDS) {
      assert.ok(ownedBy[id].length, `${id} owns no files — this test would pass vacuously for it`);
    }

    const mk = () => {
      const d = fs.mkdtempSync(path.join(os.tmpdir(), "gn-opt-"));
      execSync("git init -q", { cwd: d });
      fs.writeFileSync(path.join(d, "package.json"), '{"name":"o"}\n');
      return d;
    };

    // One install with everything, to establish that each module's files DO arrive when selected.
    // Without this half, "the files are absent" would also pass for a module that never ships.
    const all = mk();
    installKit(all, { runtime: "all", features: "all", runSetup: false, skipVerify: true });
    for (const id of FEATURE_IDS) {
      const present = ownedBy[id].filter((rel) => fs.existsSync(path.join(all, rel)));
      assert.ok(
        present.length,
        `${id}: selected in a full install and not one of its ${ownedBy[id].length} files landed`,
      );
    }
    fs.rmSync(all, { recursive: true, force: true });

    // ...and now decline each one in turn, with every other module still on.
    for (const id of FEATURE_IDS) {
      const tmp = mk();
      installKit(tmp, {
        runtime: "all",
        features: FEATURE_IDS.filter((x) => x !== id).join(","),
        runSetup: false,
        skipVerify: true,
      });
      const leaked = ownedBy[id].filter((rel) => fs.existsSync(path.join(tmp, rel)));
      assert.deepEqual(leaked, [], `${id} was declined and shipped anyway`);

      // The other nine must be unaffected — a module that takes a neighbour down with it is not
      // optional, it is entangled.
      for (const other of FEATURE_IDS.filter((x) => x !== id)) {
        assert.ok(
          ownedBy[other].some((rel) => fs.existsSync(path.join(tmp, rel))),
          `declining ${id} also removed ${other}`,
        );
      }
      fs.rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("EVERY module's contract sections vanish with it, and the rest survive", async () => {
    // The file half of optionality is above. This is the doc half: the contract is the text the
    // agent treats as authoritative, so a section describing a module that was declined is worse
    // than a leftover file — it is an instruction that cannot be followed (NS-13).
    const { FEATURE_IDS } = await import(new URL("./features.mjs", import.meta.url).href);
    ...
    const renders = {
      claude: fs.readFileSync(path.join(BUNDLE_ROOT, "templates/CLAUDE.gitnexus.md"), "utf8"),
      agents: fs.readFileSync(path.join(BUNDLE_ROOT, "templates/AGENTS.gitnexus.md"), "utf8"),
      // Two renders, not three: the Cursor rule was the third and went with that runtime. Listed
      // rather than globbed so a NEW render cannot be added without being covered here.
    };

    for (const id of FEATURE_IDS) {
      // Every heading this module owns in the canonical contract...
      const owned = [...src.matchAll(new RegExp(`<!--\\s*feature:\\s*${id}\\s*-->\\n(#{2,6} .*)`, "g"))].map(
        (m) => m[1].trim(),
      );
      // Derived from the artifact under test, so it must not be allowed to derive to NOTHING: drop
      // the tag and this loop would iterate zero headings and pass in silence, which is exactly how
      // an untagged section — one that ships to every install — would slip through. Every module
      // owns at least one section today; a module that legitimately owns none belongs in this list
      // deliberately, not by the check quietly evaporating.
      // A module with NO always-on text is a deliberate choice, not a free pass: name it here so
      // that losing a tag still fails, and so the list is a visible record of which modules reach
      // the agent only through a skill. `e2e` is delivered by files plus the bearing-e2e skill —
      // and a skill fires only when something sends the agent to it.
      const NO_CONTRACT_SECTION = new Set(["e2e"]);
      assert.equal(
        owned.length === 0,
        NO_CONTRACT_SECTION.has(id),
        owned.length
          ? `${id} is listed as having no contract section but tags ${owned.length}`
          : `${id} tags no contract section — either the tag was lost, or this check just stopped testing it`,
      );
      if (!owned.length) continue;
      const rest = new Set(FEATURE_IDS.filter((x) => x !== id));
      for (const [name, md] of Object.entries(renders)) {
        const without = filterContractByFeatures(md, rest);
        for (const heading of owned) {
          assert.ok(!without.includes(heading), `${name}: "${heading}" survived declining ${id}`);
        }
        // ...and the contract is still a document afterwards.
        assert.ok(!contractIsEmpty(without), `${name}: declining ${id} emptied the contract`);
      }
    }
  });

  it("rule packs: every pack is OWNED, and the owner gates it", async () => {
    const { featureOf, FEATURE_IDS, PACK_OWNER, PACK_DIRS, parseFeatures } = await import(
      new URL("./features.mjs", import.meta.url).href
    );
    const { shouldCopyBundleFile } = await import(
      new URL("./kit-shared.mjs", import.meta.url).href
    ...
    // Derived from the bundle, not listed here: featureOf falls through to CORE for a pack nobody
    // claims, and a core file ships everywhere — so the failure mode of forgetting a map entry is
    // a TypeScript rulebook installed silently into every Python repo, or a UI rulebook in a CLI.
    // Listing the packs in the test would go stale with the map it is meant to police (GP-11), and
    // walking PACK_DIRS means a whole new pack DIRECTORY cannot be added without being policed.
    const packs = PACK_DIRS.flatMap((dir) => {
      const abs = path.join(BUNDLE_ROOT, dir);
      if (!fs.existsSync(abs)) return [];
      return fs.readdirSync(abs).filter((f) => f.endsWith(".md")).map((f) => `${dir}${f}`);
    });
    assert.ok(packs.length > 1, "expected packs in more than one dir — the probe is wrong, not the bundle");
    for (const rel of packs) {
      const owner = PACK_OWNER[rel];
      assert.ok(owner, `${rel} has no owner — it would ship to EVERY install`);
      assert.ok(
        FEATURE_IDS.includes(owner),
        `${rel} is owned by "${owner}", which is not an installable feature`,
      );
      assert.equal(featureOf(rel), owner);
      // The feature axis must actually gate it, in both directions.
      const other = FEATURE_IDS.find((id) => id !== owner);
      assert.equal(shouldCopyBundleFile(rel, "claude", parseFeatures(owner)), true);
      assert.equal(shouldCopyBundleFile(rel, "claude", parseFeatures(other)), false);
    }
    assert.equal(featureOf("skills/bearing-tsjs/SKILL.md"), "tsjs");
    assert.equal(featureOf("skills/bearing-frontend/SKILL.md"), "frontend");
  });

  it("every shape rule says what the COMPILER does differently, or it is a style preference", () => {
    // The trap sections have a bar that enforces itself: a rule only belongs there if tsc and
    // ESLint both stay silent. "Shapes" has no such natural gate — the alternative compiles either
    // way — so the bar is the `Cashes out as:` line, and a bar that is only remembered is how a
    // rulebook turns into a style guide and stops being read.
    const pack = fs.readFileSync(
      path.join(BUNDLE_ROOT, ".bearing/lang/typescript.md"),
      "utf8",
    );
    const heading = "## Shapes that make the compiler do the work";
    const start = pack.indexOf(heading);
    assert.ok(start > 0, "the shapes section is gone");
    const body = pack.slice(start + heading.length);

    // Split on the rule marker so each rule carries its whole body, sub-bullets included.
    const rules = body.split(/\n- \*\*TS-/).slice(1);
    assert.ok(rules.length, "the shapes section has no rules — the split is wrong, not the file");
    for (const r of rules) {
      const id = `TS-${r.match(/^\d+/)?.[0]}`;
      for (const part of ["**When:**", "**Not when:**", "**Cashes out as:**"]) {
        assert.ok(
          r.includes(part),
          `${id} is missing its ${part} — without it nothing separates a shape rule from a preference`,
        );
      }
    }

    // Every TS-# the skill cites must exist, in either kind of section. A review table pointing at
    // a rule number that was renumbered away sends the agent looking for nothing.
    const defined = new Set((pack.match(/^- \*\*TS-\d+\*\*/gm) ?? []).map((m) => m.match(/TS-\d+/)[0]));
    const skill = fs.readFileSync(
      path.join(BUNDLE_ROOT, "skills/bearing-tsjs/SKILL.md"),
      "utf8",
    );
    for (const cite of skill.match(/TS-\d+/g) ?? []) {
      assert.ok(defined.has(cite), `the skill cites ${cite}, which the pack does not define`);
    }
  });

  it("a react-only install stands alone, and the frontend pack stays framework-neutral", async () => {
    // The two packs sit in the same directory and are deliberately opposite: `frontend` may name no
    // framework, `react` exists to name one. Installing either must not drag in the other, or the
    // neutrality bar on the first is decorative.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-react-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"r"}\n');
    installKit(tmp, { runtime: "claude", features: "react", runSetup: false, skipVerify: true });

    const has = (rel) => fs.existsSync(path.join(tmp, rel));
    assert.ok(has(".bearing/stack/react.md"), "the pack did not land");
    assert.ok(
      has(".claude/skills/bearing-react/SKILL.md"),
      "the skill is not readable where Claude Code looks",
    );
    assert.equal(has(".bearing/stack/frontend.md"), false, "frontend shipped with react");
    assert.equal(has(".bearing/lang/typescript.md"), false, "the TS pack shipped with react");
    assert.match(fs.readFileSync(path.join(tmp, "CLAUDE.md"), "utf8"), /REACT-1/);
    uninstallKit(tmp);
    assert.equal(has(".bearing/stack/react.md"), false, "the pack survived uninstall");
    fs.rmSync(tmp, { recursive: true, force: true });

    // Standing alone: no citation of a file another module installs.
    const contract = fs.readFileSync(
      new URL("../scripts/contract/enforcement-contract.md", import.meta.url),
      "utf8",
    );
    const start = contract.indexOf("<!-- feature: react -->");
    assert.ok(start > 0, "the react contract section is gone");
    const surfaces = {
      "contract section": contract.slice(start, contract.indexOf("<!-- feature:", start + 10)),
      pack: fs.readFileSync(path.join(BUNDLE_ROOT, ".bearing/stack/react.md"), "utf8"),
      skill: fs.readFileSync(path.join(BUNDLE_ROOT, "skills/bearing-react/SKILL.md"), "utf8"),
    };
    for (const [name, text] of Object.entries(surfaces)) {
      assert.doesNotMatch(text, /GP-\d|UI-\d|TS-\d/, `${name} cites a rule another module installs`);
      assert.doesNotMatch(text, /npm run /, `${name} names an npm script react does not create`);
      assert.doesNotMatch(text, /gitnexus_|cypher|agent-refresh/i, `${name} leaks a graph tool`);
    }

    // ...and the sections do not leak into each other's installs.
    const { filterContractByFeatures } = await import(
      new URL("./contract-filter.mjs", import.meta.url).href
    );
    const md = fs.readFileSync(path.join(BUNDLE_ROOT, "templates/CLAUDE.gitnexus.md"), "utf8");
    assert.doesNotMatch(filterContractByFeatures(md, new Set(["frontend"])), /REACT-\d/);
    assert.doesNotMatch(filterContractByFeatures(md, new Set(["react"])), /UI-\d/);
  });

  it("a frontend-only install delivers the pack and the skill, and names no framework", () => {
    // Same shape as the tsjs install test, and for the same reason: the module is only real if a
    // REAL install puts the rules where the agent reads them.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-fe-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"f"}\n');
    installKit(tmp, { runtime: "claude", features: "frontend", runSetup: false, skipVerify: true });

    const has = (rel) => fs.existsSync(path.join(tmp, rel));
    assert.ok(has(".bearing/stack/frontend.md"), "the pack did not land");
    assert.ok(
      has(".claude/skills/bearing-frontend/SKILL.md"),
      "the skill is not readable where Claude Code looks — an agent will report frontend as unavailable",
    );
    const claudeMd = fs.readFileSync(path.join(tmp, "CLAUDE.md"), "utf8");
    assert.match(claudeMd, /\.bearing\/stack\/frontend\.md/);
    assert.match(claudeMd, /UI-1/);

    // Declining the other modules must not smuggle their files in.
    assert.equal(has(".bearing/lang/typescript.md"), false, "the TS pack shipped with frontend");
    assert.equal(has(".bearing/gold-practices.md"), false, "gold practices shipped with frontend");

    assert.ok(readManifest(tmp).data.files.includes(".bearing/stack/frontend.md"));
    uninstallKit(tmp);
    assert.equal(has(".bearing/stack/frontend.md"), false, "the pack survived uninstall");
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("the frontend module stands alone, and outlives whichever framework you use", async () => {
    // Two independent bars. STANDING ALONE is the NS-13 rule every module obeys: a tsjs-style
    // citation of a file another module installs is a confident pointer to nothing.
    //
    // NAMING NO FRAMEWORK is this pack's own bar. React, Vue and Svelte churn on a two-year cycle
    // while the thing the rules are about — a shared unit has many callers, and only one of them is
    // on your screen — does not. A pack that names one dates itself and stops being read.
    const contract = fs.readFileSync(
      new URL("../scripts/contract/enforcement-contract.md", import.meta.url),
      "utf8",
    );
    const start = contract.indexOf("<!-- feature: frontend -->");
    assert.ok(start > 0, "the frontend contract section is gone");
    const section = contract.slice(start, contract.indexOf("<!-- feature:", start + 10));

    const surfaces = {
      "contract section": section,
      pack: fs.readFileSync(path.join(BUNDLE_ROOT, ".bearing/stack/frontend.md"), "utf8"),
      skill: fs.readFileSync(path.join(BUNDLE_ROOT, "skills/bearing-frontend/SKILL.md"), "utf8"),
    };
    for (const [name, text] of Object.entries(surfaces)) {
      assert.doesNotMatch(text, /GP-\d/, `${name} cites a GP-# that only goldpractices installs`);
      assert.doesNotMatch(text, /npm run /, `${name} names an npm script frontend does not create`);
      assert.doesNotMatch(text, /gitnexus_|cypher|agent-refresh/i, `${name} leaks a graph tool`);
      // Frameworks, and the CSS libraries that go stale with them.
      assert.doesNotMatch(
        text,
        /\b(React|Vue|Svelte|Angular|Next\.js|Tailwind|Bootstrap|styled-components)\b/,
        `${name} names a framework — it will outlive the rules`,
      );
    }
    // ...and the section ships only with the module.
    const { filterContractByFeatures } = await import(
      new URL("./contract-filter.mjs", import.meta.url).href
    );
    const md = fs.readFileSync(path.join(BUNDLE_ROOT, "templates/CLAUDE.gitnexus.md"), "utf8");
    assert.match(filterContractByFeatures(md, new Set(["frontend"])), /UI-1/);
    assert.doesNotMatch(filterContractByFeatures(md, new Set(["tsjs"])), /UI-1/);
  });

  it("tsjs stands alone — it cites nothing another module installs", async () => {
    // "Each feature must stand alone" (features.mjs header). gold-practices ships with NORTHSTARS,
    // so a `GP-#` citation in the TS pack is a pointer to a file a tsjs-only repo does not have —
    // the NS-13 failure, in the doc the agent treats as authoritative.
    const { filterContractByFeatures } = await import(
      new URL("./contract-filter.mjs", import.meta.url).href
    );
    const contract = fs.readFileSync(
    ...
    const start = contract.indexOf("<!-- feature: tsjs -->");
    assert.ok(start > 0, "the tsjs contract section is gone");
    const section = contract.slice(start, contract.indexOf("<!-- feature:", start + 10));

    const surfaces = {
      "contract section": section,
      "pack": fs.readFileSync(path.join(BUNDLE_ROOT, ".bearing/lang/typescript.md"), "utf8"),
      "skill": fs.readFileSync(path.join(BUNDLE_ROOT, "skills/bearing-tsjs/SKILL.md"), "utf8"),
    };
    for (const [name, text] of Object.entries(surfaces)) {
      assert.doesNotMatch(text, /GP-\d/, `${name} cites a GP-# that only northstars installs`);
      assert.doesNotMatch(text, /gold-practices/, `${name} names a file only northstars installs`);
      assert.doesNotMatch(text, /npm run /, `${name} names an npm script tsjs does not create`);
      assert.doesNotMatch(text, /gitnexus_|cypher|agent-refresh/i, `${name} leaks a graph tool`);
    }

    // The section ships with the module and vanishes without it.
    const md = fs.readFileSync(path.join(BUNDLE_ROOT, "templates/CLAUDE.gitnexus.md"), "utf8");
    assert.match(filterContractByFeatures(md, new Set(["tsjs"])), /\.bearing\/lang\/typescript\.md/);
    const without = filterContractByFeatures(md, new Set(["northstars", "taskcore"]));
    assert.doesNotMatch(without, /\.bearing\/lang\//);
    assert.doesNotMatch(without, /bearing-tsjs/);
  });

  it("a tsjs-only install delivers the pack and the skill, and nothing else's files", () => {
    // Unit-testing the filter is not enough — the module is only real if a REAL install puts the
    // rules where the agent reads them (the 1.1.2 lesson: a module whose skill cannot be read is
    // reported as not existing).
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-tsjs-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"t"}\n');
    installKit(tmp, { runtime: "claude", features: "tsjs", runSetup: false, skipVerify: true });

    const has = (rel) => fs.existsSync(path.join(tmp, rel));
    assert.ok(has(".bearing/lang/typescript.md"), "the pack did not land");
    assert.ok(has(".bearing/skills/bearing-tsjs/SKILL.md"), "the skill is not in the store");
    assert.ok(
      fs.existsSync(path.join(tmp, ".claude/skills/bearing-tsjs/SKILL.md")),
      "the skill is not readable where Claude Code looks — an agent will report tsjs as unavailable",
    );
    // The contract the agent actually reads must carry the rules.
    const claudeMd = fs.readFileSync(path.join(tmp, "CLAUDE.md"), "utf8");
    assert.match(claudeMd, /\.bearing\/lang\/typescript\.md/);
    assert.match(claudeMd, /TS-1/);

    // Declining northstars must not smuggle its file in through the TS module.
    assert.equal(has(".bearing/gold-practices.md"), false, "gold-practices shipped without northstars");
    assert.equal(has(".claude/hooks/bearing-grep-guard.mjs"), false, "a graph gate shipped");

    // Uninstall owns what it installed: the pack is manifest-tracked, not left behind.
    assert.ok(readManifest(tmp).data.files.includes(".bearing/lang/typescript.md"));
    uninstallKit(tmp);
    assert.equal(has(".bearing/lang/typescript.md"), false, "the pack survived uninstall");
  });
  ...
  it("update-all preserves each repo's own module selection", async () => {
    ...
    for (const [name, feats] of SPECS) {
      ...
      assert.deepEqual(
        [...m.features].sort(),
        [...inheritSplitFeatures(new Set(feats.split(",")))].sort(),
  ...
  it("a run of EDITS nudges toward invasive minions, not the gathering ones", () => {
    // The same blindness costs more when the work is a WRITE: forty files edited by hand, each edit
    // identical in shape, while the thinking was done before the first one. Gathering and writing
    // are counted as SEPARATE runs — a long read run followed by a long edit run is how this work
    // actually arrives, and one shared counter would spend the budget on the wrong skill.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-invasive-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"n"}\n');
    installKit(tmp, { runtime: "claude", features: "minions", runSetup: false, skipVerify: true });
    fs.writeFileSync(
    ...
    const fire = (tool, target) => {
      ...
      return {
        status: r.status,
        invasive: /bearing-invasive-minions/.test(out),
        gathering: /load the `bearing-minions` skill/.test(out),
      };
    };

    // A run of edits across DISTINCT files → the invasive skill, once.
    assert.equal(fire("Edit", "/a.ts").invasive, false);
    assert.equal(fire("Edit", "/b.ts").invasive, false);
    const third = fire("Edit", "/c.ts");
    assert.equal(third.invasive, true, "no invasive nudge at the threshold");
    assert.equal(third.gathering, false, "an edit run pointed at the GATHERING skill");
    assert.equal(third.status, 0, "the nudge blocked a tool call (NS-5)");
    assert.equal(fire("Edit", "/d.ts").invasive, false, "nudged twice — once per session");

    // A run of reads still points at the gathering skill. The two budgets are independent, so the
    // edit nudge above must not have spent the read one.
    reset();
    fire("Read", "/p.ts");
    fire("Read", "/q.ts");
    const r3 = fire("Read", "/r.ts");
    assert.equal(r3.gathering, true, "the gathering nudge was consumed by the edit run");
    assert.equal(r3.invasive, false, "a read run pointed at the INVASIVE skill");

    // NS-12: editing ONE file repeatedly is iteration, not a fan-out. Counting calls rather than
    // distinct files would nag through every normal edit loop.
    reset();
    for (let i = 0; i < 6; i++) {
      assert.equal(fire("Edit", "/same.ts").invasive, false, "one file counted as a new site");
    }

    // An edit with no file_path cannot be told from the next one, so it must not count at all —
    // and the target fallback (`pattern`, then the tool name) would otherwise let a varying field
    // masquerade as a distinct SITE. Vary that field deliberately: with the guard, nothing counts;
    // without it, three "sites" appear and the nudge fires on work that touched no new file.
    reset();
    for (const p of ["one", "two", "three", "four"]) {
      const r = spawnSync(process.execPath, [hook], {
        cwd: tmp,
        encoding: "utf8",
        input: JSON.stringify({ cwd: tmp, tool_name: "Edit", tool_input: { pattern: p } }),
        env: { ...process.env, CLAUDE_PROJECT_DIR: tmp },
      });
      assert.ok(
        !/bearing-invasive-minions/.test(r.stdout || ""),
        "an edit with no file_path counted as a distinct site",
      );
    }

    // Delegating clears the edit run too — the agent is already doing the right thing.
    reset();
    fire("Edit", "/x.ts");
    fire("Edit", "/y.ts");
    fire("Task", "");
    assert.equal(fire("Edit", "/z.ts").invasive, false, "the edit run survived a delegation");

    // And the same switch disables both.
    reset();
    fs.writeFileSync(
      path.join(tmp, ".bearing/hooks.local.json"),
      JSON.stringify({ minionFanoutThreshold: 0 }),
    );
    for (const t of ["/1.ts", "/2.ts", "/3.ts", "/4.ts"]) {
      assert.equal(fire("Edit", t).invasive, false, "threshold 0 must disable the invasive nudge");
    }
    fs.rmSync(tmp, { recursive: true, force: true });
  });
  ...
  it("gold practices are bearing's file, refresh on update, and never outrank the project's own", () => {
    ...
    // Its own module now: selecting north-stars is NOT selecting gold practices, in either
    // direction. The two are still read together, which is a precedence rule rather than a package.
    installKit(tmp, { runtime: "claude", features: "northstars,taskcore", runSetup: false, skipVerify: true });
    assert.ok(
      !fs.existsSync(path.join(tmp, ".bearing/gold-practices.md")),
      "gold practices shipped to a repo that declined the module",
    );

    installKit(tmp, {
      runtime: "claude",
      features: "goldpractices,northstars,taskcore",
      runSetup: false,
      skipVerify: true,
    });
    const gp = path.join(tmp, ".bearing/gold-practices.md");
    assert.ok(fs.existsSync(gp), "the gold practices module did not bring its own file");
  ...
  it("still finds and migrates an install whose manifest is at the legacy path", () => {
    ...
    // Still north-stars, plus whatever has since been split out of it — that inheritance is what
    // stops the upgrade deleting a file this repo had opted into. Anything ELSE appearing here is
    // the failure the assertion is for.
    assert.deepEqual(
      [...(readManifest(tmp)?.data.features ?? [])].sort(),
      [...inheritSplitFeatures(new Set(["northstars"]))].sort(),
    );
  ...
  it("bearing changes only the files it owns: modes, MCP transport, and no-op JSON writes", () => {
    // Three ways an update dirtied a repo without being asked, all found by reading a real diff:
    // every `.sh` in the tree was chmodded to 755, a committed `.mcp.json` had its transport
    // rewritten from a machine-local manifest, and JSON we merely merge into came back reformatted.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-notours-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), JSON.stringify({ name: "t" }));
    // A committed .mcp.json, written the way a person writes one — compact, stdio.
    fs.writeFileSync(
      path.join(tmp, ".mcp.json"),
      '{\n  "mcpServers": {\n    "gitnexus": { "command": "gitnexus", "args": ["mcp"] }\n  }\n}\n',
    );
    fs.mkdirSync(path.join(tmp, "scripts"), { recursive: true });
    const theirs = path.join(tmp, "scripts/project-tool.sh");
    fs.writeFileSync(theirs, "#!/bin/sh\necho theirs\n");
    fs.chmodSync(theirs, 0o644);
    execSync("git add -A && git -c user.email=t@t -c user.name=t commit -qm i", { cwd: tmp });
    const committedMcp = fs.readFileSync(path.join(tmp, ".mcp.json"), "utf8");
    ...
    // MODE: theirs untouched, ours executable.
    assert.equal(fs.statSync(theirs).mode & 0o777, 0o644, "chmodded a script bearing does not own");
    assert.equal(
      fs.statSync(path.join(tmp, "scripts/bearing-setup.sh")).mode & 0o777,
      0o755,
      "our own script is not executable",
    );

    // TRANSPORT + FORMATTING: the committed file decides, and an unchanged result is not rewritten.
    assert.equal(
      fs.readFileSync(path.join(tmp, ".mcp.json"), "utf8"),
      committedMcp,
      "a byte of the committed .mcp.json changed when nothing about it needed to",
    );

    // The machine-local manifest must NOT outrank the committed file on a later update. This is the
    // shape that flipped a real repo: manifest says http, the team's committed file says stdio.
    const mf = path.join(tmp, MANIFEST_PATH);
    const m = JSON.parse(fs.readFileSync(mf, "utf8"));
    m.mcpTransport = { mode: "http", url: "http://127.0.0.1:39100/mcp" };
    fs.writeFileSync(mf, JSON.stringify(m));
    updateKit(tmp, { runSetup: false, skipVerify: true });
    assert.equal(
      fs.readFileSync(path.join(tmp, ".mcp.json"), "utf8"),
      committedMcp,
      "a per-machine transport rewrote a team-shared file",
    );

    // But an EXPLICIT choice on this run still wins — preservation must not become a cage (NS-6).
    updateKit(tmp, { mcpTransport: "http", runSetup: false, skipVerify: true });
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(tmp, ".mcp.json"), "utf8")).mcpServers.gitnexus.type,
      "http",
      "--mcp http no longer overrides",
    );
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("north-stars: seeded once, never overwritten, and never deleted once authored", async () => {
    // Five of ten real installs had NO northstars.md, so the primer said nothing, the anchor took
    // its "no NS-# lines" exit on every fire, and a selected module did nothing at all. bearing now
    // seeds a starter — which drags the file into the manifest, and the manifest is what uninstall
    // deletes. NS-1 is named after uninstall deleting this exact file.
    const sp = await import(
      new URL("../bundle/.bearing/lib/session-primer.mjs", import.meta.url).href
    );
    const NS = ".bearing/northstars.md";
    const mk = () => {
      const t = fs.mkdtempSync(path.join(os.tmpdir(), "gn-nsseed-"));
      execSync("git init -q", { cwd: t });
      fs.writeFileSync(path.join(t, "package.json"), JSON.stringify({ name: "x" }));
      execSync("git add -A && git -c user.email=t@t -c user.name=t commit -qm i", { cwd: t });
      installKit(t, { runtime: "claude", runSetup: false, skipVerify: true, quick: true });
      return t;
    };

    // Seeded, owned, and — critically — carrying NOTHING citable. A starter with a real NS-# would
    // be re-anchored mid-session as though bearing had decided something about someone's project.
    const a = mk();
    assert.ok(fs.existsSync(path.join(a, NS)), "the module installed no north-stars file");
    assert.equal(sp.northStarsDigest(a).length, 0, "the shipped starter is citable as fact");
    assert.ok(sp.northStarsExists(a), "seeded file must read as present");
    assert.ok((readManifest(a)?.data.files ?? []).includes(NS), "seeded file not recorded");
    assert.doesNotMatch(fs.readFileSync(path.join(a, NS), "utf8"), /__[A-Z_]+__/, "placeholder leaked");

    // SEED-ONCE: an update must not touch a file the user has written in.
    const mine = "\n- **NS-1** — **Mine.** Irreplaceable, and never committed.\n";
    fs.appendFileSync(path.join(a, NS), mine);
    updateKit(a, { runSetup: false, skipVerify: true });
    assert.match(fs.readFileSync(path.join(a, NS), "utf8"), /Irreplaceable/, "update overwrote it");
    assert.equal(sp.northStarsDigest(a).length, 1);
    fs.rmSync(a, { recursive: true, force: true });

    // Both removal paths must spare an AUTHORED file — in both NS-# spellings. If this check's
    // pattern and the digest's ever disagree, a file the anchor reads happily becomes a file the
    // installer deletes, so the heading form is asserted here deliberately.
    for (const [label, claim] of [
      ["bullet", "\n- **NS-1** — **Mine.** Irreplaceable.\n"],
      ["heading", "\n### NS-1 — Mine, written as a heading.\n"],
    ]) {
      for (const [how, run] of [
        ["deselect", (t) => updateKit(t, { features: "-northstars", runSetup: false, skipVerify: true })],
        ["uninstall", (t) => uninstallKit(t)],
      ]) {
        const t = mk();
        fs.appendFileSync(path.join(t, NS), claim);
        assert.ok(sp.northStarsDigest(t).length > 0, `${label}: digest cannot see it`);
        run(t);
        assert.ok(fs.existsSync(path.join(t, NS)), `${how} DELETED an authored ${label} file`);
        assert.match(fs.readFileSync(path.join(t, NS), "utf8"), /Mine/, `${how} truncated it`);
        fs.rmSync(t, { recursive: true, force: true });
      }
    }

    // A PRISTINE starter is still ours — leaving it behind would mean uninstall never cleans up.
    const c = mk();
    uninstallKit(c);
    assert.ok(!fs.existsSync(path.join(c, NS)), "an untouched starter was left behind");
    assert.ok(!fs.existsSync(path.join(c, ".bearing")), "uninstall left a .bearing/ shell");
    fs.rmSync(c, { recursive: true, force: true });

    // Declining the module installs nothing.
    const d = fs.mkdtempSync(path.join(os.tmpdir(), "gn-nsdecline-"));
    execSync("git init -q", { cwd: d });
    fs.writeFileSync(path.join(d, "package.json"), JSON.stringify({ name: "d" }));
    execSync("git add -A && git -c user.email=t@t -c user.name=t commit -qm i", { cwd: d });
    installKit(d, { runtime: "claude", features: "taskcore", runSetup: false, skipVerify: true });
    assert.ok(!fs.existsSync(path.join(d, NS)), "north-stars leaked into a repo that declined it");
    fs.rmSync(d, { recursive: true, force: true });
  });

  it("the agent-doc checks judge only the doc THIS runtime writes", () => {
    // A `runtime: claude` repo that also tracks an AGENTS.md — committed by the team, or left by an
    // earlier `--runtime all` install — reported "no persona line in AGENTS.md" and
    // "AGENTS.md: 0 current + 0 legacy block(s)", so every update ended "finished with 2 FAILED
    // checks" for a file bearing had correctly declined to touch. The headline became the false
    // claim (NS-20), which is the exact inversion of what this module is for.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-agentdocs-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), JSON.stringify({ name: "a" }));
    // An AGENTS.md that bearing did NOT write: no managed block, no persona line.
    fs.writeFileSync(path.join(tmp, "AGENTS.md"), "# Our own agent notes\n\nNothing to do with bearing.\n");
    execSync("git add -A && git -c user.email=t@t -c user.name=t commit -qm i", { cwd: tmp });
    installKit(tmp, { runtime: "claude", runSetup: false, skipVerify: true, quick: true });

    const ids = new Set(["persona", "agent_docs"]);
    const checks = (runtime) =>
      runPostChecks(tmp, { manifest: { ...readManifest(tmp)?.data, runtime } }).filter((c) =>
        ids.has(c.id),
      );

    for (const c of checks("claude")) {
      assert.ok(c.ok, `${c.label} failed over an unmanaged doc: ${c.detail}`);
      assert.doesNotMatch(c.detail, /AGENTS\.md/, "a claude install judged AGENTS.md");
    }

    // NS-12 — the check must still be ABLE to fail, on the doc this runtime does write.
    const claudeMd = path.join(tmp, "CLAUDE.md");
    const orig = fs.readFileSync(claudeMd, "utf8");
    fs.writeFileSync(claudeMd, `${orig}\n${AGENTS_MARKER_BEGIN}\na second block\n`);
    assert.ok(
      !checks("claude").find((c) => c.id === "agent_docs")?.ok,
      "a duplicated managed block in CLAUDE.md went unnoticed",
    );
    fs.writeFileSync(claudeMd, orig);

    // And a runtime that DOES write AGENTS.md must still be judged on it.
    assert.ok(
      checks("zed").some((c) => !c.ok && /AGENTS\.md/.test(c.detail)),
      "a zed install stopped checking the doc it writes",
    );
    // An unknown runtime must widen, never narrow — a missing answer must not quietly stop a check.
    assert.ok(
      checks("").some((c) => !c.ok && /AGENTS\.md/.test(c.detail)),
      "an unreadable runtime narrowed verification instead of widening it",
    );
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("update-all discovers a committed install whose manifest is gitignored — and nothing else", () => {
    // The manifest is gitignored, so a fresh clone of a repo with bearing COMMITTED has none, and
    // discovery walked straight past 97 tracked .bearing/ files. updateKit already handles that
    // repo; update-all never showed it one. Measured on a real clone: 10 repos found, the 11th
    // invisible.
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "gn-discover-"));

    // (a) bearing committed, manifest gitignored — the team case.
    const team = path.join(root, "team");
    fs.mkdirSync(team);
    execSync("git init -q", { cwd: team });
    fs.writeFileSync(path.join(team, "package.json"), JSON.stringify({ name: "t" }));
    execSync("git add -A && git -c user.email=t@t -c user.name=t commit -qm i", { cwd: team });
    installKit(team, { runtime: "claude", runSetup: false, skipVerify: true, quick: true });
    execSync(
      "git add -A -f .bearing && git -c user.email=t@t -c user.name=t commit -qm bearing",
      { cwd: team },
    );
    fs.rmSync(path.join(team, MANIFEST_PATH)); // exactly what a fresh clone looks like
    execSync(`git rm -q --cached ${MANIFEST_PATH} && git -c user.email=t@t -c user.name=t commit -qm drop`, {
      cwd: team,
    });

    // (b) a repo that never installed bearing but tracks its OWN .claude/hooks and .cursor/rules.
    // A scan acts on whatever it matches, so a false positive here installs this kit into someone
    // else's project (NS-1). This is the assertion that keeps discovery narrow (NS-12).
    const innocent = path.join(root, "innocent");
    fs.mkdirSync(path.join(innocent, ".claude/hooks"), { recursive: true });
    fs.mkdirSync(path.join(innocent, ".cursor/rules"), { recursive: true });
    execSync("git init -q", { cwd: innocent });
    fs.writeFileSync(path.join(innocent, ".claude/hooks/my-own.mjs"), "console.log(1)\n");
    fs.writeFileSync(path.join(innocent, ".cursor/rules/mine.mdc"), "my rule\n");
    execSync("git add -A && git -c user.email=t@t -c user.name=t commit -qm i", { cwd: innocent });

    const found = findInstalledRepos(root).map((p) => path.basename(p));
    assert.deepEqual(found, ["team"], `discovery returned ${JSON.stringify(found)}`);

    // And the repo it found must actually be updatable — discovery agreeing with updateKit is the
    // whole point, so a repo that appears here and then throws is a worse bug than not appearing.
    updateKit(team, { runSetup: false, skipVerify: true });
    assert.ok(readManifest(team), "discovered repo could not be updated");
    fs.rmSync(root, { recursive: true, force: true });
  });

  it("an install that selected NOTHING is not handed the whole default set by its next update", () => {
    // `[]` is an answer, not a blank. It reached parseFeatures as `""` — "nobody said" — which
    // answers with the full recommended set, so a repo that declined every module had all eleven
    // written back into it on the next update (NS-1), gates included (NS-13).
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-nonefeat-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), JSON.stringify({ name: "n" }));
    execSync("git add -A && git -c user.email=t@t -c user.name=t commit -qm i", { cwd: tmp });
    installKit(tmp, {
      runtime: "claude",
      features: "none",
    ...
    assert.deepEqual(readManifest(tmp)?.data.features, [], "--features none selected something");

    updateKit(tmp, { runSetup: false, skipVerify: true });
    assert.deepEqual(readManifest(tmp)?.data.features, [], "update reinstated declined modules");
    assert.ok(
      !fs.existsSync(path.join(tmp, ".bearing/northstars.md")),
      "update wrote a module file into a repo that selected none",
    );

    // Re-install without --features reads the same recorded set through the other call site.
    installKit(tmp, { runtime: "claude", runSetup: false, skipVerify: true, quick: true });
    assert.deepEqual(readManifest(tmp)?.data.features, [], "re-install reinstated declined modules");

    // And the empty set is still a usable base for a delta.
    updateKit(tmp, { features: "+tsjs", runSetup: false, skipVerify: true });
    assert.deepEqual(readManifest(tmp)?.data.features, ["tsjs"]);
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("a +delta on a pre-split manifest keeps the module that was split out", () => {
    // The whole fleet is pre-split: every manifest written by 1.1.x names `northstars` and cannot
    // possibly name `goldpractices`. `update` alone inherits it — but `update --features +tsjs`
    // resolved the delta against the RAW recorded set, so adding one module silently removed
    // another and deleted .bearing/gold-practices.md with the user's own PP-# rules in it (NS-1).
    // A delta is an EDIT to the recorded set, so the set it edits must inherit first.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-deltainherit-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), JSON.stringify({ name: "d" }));
    execSync("git add -A && git -c user.email=t@t -c user.name=t commit -qm i", { cwd: tmp });
    installKit(tmp, { runtime: "claude", runSetup: false, skipVerify: true, quick: true });

    // Rewrite the manifest to exactly what 1.1.x left on disk, and put a user-owned rule in the
    // file — losing THAT is the damage, not losing a manifest entry.
    const mf = path.join(tmp, MANIFEST_PATH);
    const m = JSON.parse(fs.readFileSync(mf, "utf8"));
    m.features = m.features.filter((f) => f !== "goldpractices");
    fs.writeFileSync(mf, JSON.stringify(m));
    const gp = path.join(tmp, ".bearing/gold-practices.md");
    fs.appendFileSync(gp, "\n- **PP-1** — a rule the user wrote themselves.\n");

    updateKit(tmp, { features: "+tsjs", runSetup: false, skipVerify: true });

    const after = readManifest(tmp)?.data.features ?? [];
    assert.ok(after.includes("tsjs"), "the delta did not apply");
    assert.ok(after.includes("goldpractices"), "adding a module silently dropped an inherited one");
    assert.ok(fs.existsSync(gp), "the update deleted the user's gold-practices file");
    assert.match(fs.readFileSync(gp, "utf8"), /PP-1/, "the user's own rule was lost");

    // A signed removal still wins: inheritance grants the id, the delta may then take it away.
    updateKit(tmp, { features: "-goldpractices", runSetup: false, skipVerify: true });
    assert.ok(
      !(readManifest(tmp)?.data.features ?? []).includes("goldpractices"),
      "inheritance overrode an explicit opt-out",
    );
    fs.rmSync(tmp, { recursive: true, force: true });
  });
  ...
  it("a run's evidence survives the exit — the context is closed before the process ends", async () => {
    // THE scar. `finish()` scheduled an exit on a microtask that resolved before teardown had done
    // one IPC round trip, so `context.close()` never ran — and Playwright writes a recorded video
    // ONLY on context close. Every passing run produced a **0-byte .webm**: a file that exists,
    // will not play, and reads as success. Two reviewers reproduced it 5/5 before anyone noticed,
    // because the check counted the file and never its size.
    //
    // This drives the REAL exit path (`exit: true`, `process.exit` stubbed) and asserts the
    // ORDERING, because asserting that a video exists would pass on a 0-byte one.
    const require = createRequire(import.meta.url);
    const recording = require("../bundle/.e2e/core/recording.js");
    const { createReport } = require("../bundle/.e2e/core/report.js");

    const events = [];
    // A close that is SLOW, the way a real IPC round trip is: an exit that does not truly wait for
    // it lands first, which is precisely the defect.
    const context = {
      on() {},
      close: () => new Promise((r) => setTimeout(() => { events.push("context-closed"); r(); }, 60)),
    };
    recording.adopt(context, { isClosed: () => false }, { take: async () => ({ path: "x.png" }) });

    const report = createReport("ordering");
    const logged = console.log;
    const realExit = process.exit;
    console.log = () => {};
    try {
      const exited = new Promise((resolve) => {
        process.exit = (code) => { events.push("exit"); resolve(code); };
      });
      report.check("something", true);
      report.finish();
      assert.equal(await exited, 0, "a passing run did not exit 0");
    } finally {
      process.exit = realExit;
      console.log = logged;
    }
    assert.deepEqual(events, ["context-closed", "exit"], "the process exited before the video was written");
  });

  it("a second drain joins the first rather than returning early", async () => {
    // The same defect one level down, and the reason it was intermittent: the first drain took the
    // contexts and cleared the set, so a concurrent second drain found nothing to do and resolved
    // immediately — after which `browser.close()` killed the context mid-write. One run in three
    // produced a real file, so the feature looked like it worked.
    //
    // Awaiting ONLY the second drain is the whole point: awaiting both would pass either way.
    const require = createRequire(import.meta.url);
    const recording = require("../bundle/.e2e/core/recording.js");
    let closed = false;
    const context = {
      on() {},
      close: () => new Promise((r) => setTimeout(() => { closed = true; r(); }, 60)),
    };
    recording.adopt(context, { isClosed: () => false }, { take: async () => ({}) });
    const first = recording.drain();
    await recording.drain();
    assert.ok(closed, "the second drain resolved before the context had finished closing");
    await first;
  });

  it("an implemented session contract stub survives the next update", () => {
    // `.e2e/core/session.js` ships as a CONTRACT STUB whose own docblock says "bearing ships the
    // shape and the scars; you write the body" — and the next update deleted the body. How an app
    // holds a session is the most app-specific thing about it, so that file is hours of someone's
    // work: the storage keys, whether values are stored raw, which of localStorage and
    // sessionStorage the app's reader actually reads.
    //
    // Overwriting it does not merely lose work. It returns the repo to the exact state the stub
    // exists to warn about — a verifier running logged-out, reporting every feature as missing,
    // which reads as a broken app rather than a broken harness (NS-1).
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-stub-"));
    execSync("git init -q", { cwd: tmp });
    fs.writeFileSync(path.join(tmp, "package.json"), '{"name":"s"}\n');
    execSync("git add -A && git -c user.email=t@t -c user.name=t commit -qm i", { cwd: tmp, shell: "/bin/bash" });
    installKit(tmp, { runtime: "claude", features: "e2e", runSetup: false, skipVerify: true, quick: true });

    const session = path.join(tmp, ".e2e/core/session.js");
    assert.ok(fs.existsSync(session), "the e2e module shipped no session stub");
    const MINE = "\n// MY IMPLEMENTATION — the app's real storage keys\n";
    fs.appendFileSync(session, MINE);

    updateKit(tmp, { runSetup: false, skipVerify: true });
    assert.match(
      fs.readFileSync(session, "utf8"),
      /MY IMPLEMENTATION/,
      "the update overwrote an implemented contract stub",
    );
    // Idempotent: a second update must not take it either (NS-3).
    updateKit(tmp, { runSetup: false, skipVerify: true });
    assert.match(fs.readFileSync(session, "utf8"), /MY IMPLEMENTATION/, "the second update took it");
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("a PASS can be photographed too, because a wrong pass is the invisible one", async () => {
    // The first version captured only failures, which has the asymmetry backwards: a FAIL already
    // tells you to go and look, while a PASS that is wrong tells you nothing and is believed. Every
    // locator is a claim about the DOM, and one that stops matching does not raise its hand — it
    // reports ABSENCE, which reads as a clean pass. That is the expensive direction and it was the
    // unphotographed one (app-desktop's PP-21: photograph the moments a verdict RESTS on).
    const require = createRequire(import.meta.url);
    const recPath = path.join(BUNDLE_ROOT, ".e2e", "core", "recording.js");

    const load = (tier) => {
      for (const k of Object.keys(require.cache)) delete require.cache[k];
      const prev = process.env.SHOTS;
      if (tier === undefined) delete process.env.SHOTS;
      else process.env.SHOTS = tier;
      const rec = require(recPath);
      if (prev === undefined) delete process.env.SHOTS;
      else process.env.SHOTS = prev;
      return rec;
    };

    // Default: a pass is NOT photographed. A frame per passing assertion is thousands of files on a
    // real suite, and noise that large is indistinguishable from no evidence at all.
    assert.equal(load("key").onPass("a check", ""), null, "the default tier photographed every pass");
    assert.equal(load(undefined).onPass("a check", ""), null, "no SHOTS set photographed every pass");

    // ...unless THIS check says its verdict rests on something visible. That is the knowledge the
    // check's author has and the tier does not.
    assert.notEqual(
      load("key").onPass("a check", "", true),
      null,
      "{ evidence: true } did not photograph a pass",
    );

    // A run being investigated turns it on for everything.
    assert.notEqual(load("evidence").onPass("a check", ""), null, "SHOTS=evidence skipped a pass");
    assert.notEqual(load("all").onPass("a check", ""), null, "SHOTS=all skipped a pass");

    // `off` still means off — it exists for a check that MEASURES timing, where the shutter moves
    // the number being reported, so even a forced evidence frame must not fire.
    assert.equal(await load("off").onPass("a check", "", true), null, "SHOTS=off took a frame anyway");

    // And the wiring: report.check routes a pass to onPass and a failure to onFailure.
    const src = fs.readFileSync(path.join(BUNDLE_ROOT, ".e2e", "core", "report.js"), "utf8");
    assert.match(src, /if \(!ok\) recording\.onFailure\(name, detail\);/);
    assert.match(src, /else recording\.onPass\(name, detail, opts\.evidence === true\);/);
  });

  it("the fixture ledger refuses everything that is not demonstrably a creation", async () => {
    // This module decides what a DESTRUCTIVE tool will later delete, so every default errs toward
    // under-tracking: a missed fixture is visible litter, an over-tracked one is an irreversible
    // delete. Each refusal below was a live hazard in the first draft.
    //
    // ⚠ THE POSITIVE CASE IS ASSERTED FIRST, AND IT IS NOT DECORATION. When these refusals were
    // first run, all four passed while `track()` was throwing on every call — "it refused
    // everything" and "it is broken" produce identical output, and only a case that must be TRACKED
    // can tell them apart.
    const require = createRequire(import.meta.url);
    const seeds = require("../bundle/.e2e/core/seeds.js");
    const fsx = require("node:fs");
    fsx.rmSync(seeds.DIR, { recursive: true, force: true });

    const res = ({ method = "POST", status = 201, url = "https://app.test/api/things", served = true }) => ({
      status: () => status,
      url: () => url,
      request: () => ({ method: () => method }),
      serverAddr: async () => (served ? { ipAddress: "::1", port: 443 } : null),
      json: async () => ({ id: "REAL-42", name: "a fixture" }),
    });
    const handlers = [];
    const page = { url: () => "https://app.test/", on: (_e, fn) => handlers.push(fn) };
    seeds.watchPage(page);
    const fire = async (r) => { handlers.forEach((h) => h(r)); await seeds.settle(); };

    await fire(res({}));
    assert.deepEqual(
      seeds.outstanding().entries.map((e) => e.url),
      ["https://app.test/api/things/REAL-42"],
      "a served 201 on the page's own origin was not ledgered — the refusals below prove nothing",
    );

    const refused = async (label, r) => {
      const before = seeds.outstanding().entries.length;
      await fire(r);
      assert.equal(seeds.outstanding().entries.length, before, label);
    };
    // route.fulfill() produces a REAL response event; blockWrites is the kit's own recommendation,
    // so the ledger would record a phantom id and the sweeper would delete a real object of it.
    await refused("an intercepted response was ledgered", res({ served: false, url: "https://app.test/api/x" }));
    // 200 on a POST is routinely an idempotent create — deleting that destroys a pre-existing record.
    await refused("a 200 create was ledgered", res({ status: 200, url: "https://app.test/api/y" }));
    // A payments app POSTs to Stripe on ordinary loads, and DELETE /v1/customers/{id} is a real route.
    await refused("a third-party origin was ledgered", res({ url: "https://api.stripe.com/v1/customers" }));
    await refused("a token refresh was ledgered", res({ url: "https://app.test/auth/refresh" }));

    // A clean run must leave nothing, so a DELETE releases the entry — and the two spellings of the
    // same URL must match, which is why normalize() is shared by every read and write.
    await fire(res({ method: "DELETE", status: 204, url: "https://app.test/api/things/REAL-42/" }));
    assert.deepEqual(seeds.outstanding().entries, [], "a DELETE did not release its entry");
    fsx.rmSync(seeds.DIR, { recursive: true, force: true });
  });

  it("audit-checks fires on every smell, and stays silent on the code that only looks like one", () => {
    // ⚠ THE POINT OF THE FIXTURES. This tool's first version shipped with a docblock claiming a
    // negative-control folder that did not exist, so no rule had ever been watched to fire — by its
    // own stated standard, it was not known to work. Three of its rules turned out to fail a branch
    // on CORRECT code, which for a linter is the expensive direction to be wrong in.
    //
    // Asserting rule ids rather than a count: a rule that silently stops matching, while another
    // over-matches by the same amount, keeps the total honest and the tool useless.
    const require = createRequire(import.meta.url);
    const { execFileSync } = require("node:child_process");
    const dir = path.join(BUNDLE_ROOT, ".e2e", "tools", "audit-fixtures");
    const tool = path.join(BUNDLE_ROOT, ".e2e", "tools", "audit-checks.js");

    let out = "";
    try {
      out = execFileSync(process.execPath, [tool], { env: { ...process.env, AUDIT_DIR: dir }, encoding: "utf8" });
    } catch (err) {
      // Exit 1 is expected — dirty.js carries LIE findings, which is what gates a branch.
      out = err.stdout || "";
    }
    ...
    assert.deepEqual(
      idsOn("dirty.js"),
      [
        "fixtures-without-a-ledger",
        "playwright-syntax-in-evaluate",
        "report-never-finished",
        "undebuggable-assertion",
        "vacuous-assertion",
      ],
      "a rule stopped firing on the file written to trip every one of them",
    );
    // Each of these was a real false positive: `:has()` is standard CSS, `page.check()` is
    // Playwright's own, a comment describing a smell is not one, a string containing `/*` is not a
    // comment, and `report?.finish?.()` still finishes.
    assert.deepEqual(idsOn("traps.js"), [], "audit-checks flagged code that only looks like a smell");
    assert.deepEqual(idsOn("clean.js"), [], "audit-checks flagged the correct form of each smell");
    assert.deepEqual(idsOn("suppressed.js"), [], "the ignore comment did not suppress");
  });

  it("the sweeper treats a process it may not signal as ALIVE, not as dead", () => {
    // `process.kill(pid, 0)` throws two different things and the first version collapsed them:
    // ESRCH is "no such process", EPERM is "it is there and you may not signal it" — a ledger
    // written under another uid, which is ordinary on a shared CI runner or across a container
    // boundary. Reading EPERM as dead made the one DESTRUCTIVE tool in this kit sweep the fixtures
    // of a verifier that was still running, which is exactly the "the next run reports a plausible
    // product defect" failure the ledger exists to end (NS-1).
    //
    // Nothing covered this: inverting the guard entirely left the whole suite green.
    const require = createRequire(import.meta.url);
    const seeds = require("../bundle/.e2e/core/seeds.js");
    const fsx = require("node:fs");
    const { execFileSync } = require("node:child_process");

    // PID 1 always exists and is never ours to signal — unless the suite is running as root, in
    // which case there is no EPERM to be had. Say so rather than passing on a case never exercised
    // (NS-12: "looked and found nothing" is not "never ran the check").
    let errno = null;
    try {
      process.kill(1, 0);
    } catch (err) {
    ...
    if (errno !== "EPERM") {
      console.log(`      (skipped: process.kill(1,0) gave ${errno ?? "no error"}, not EPERM — running as root?)`);
      return;
    }

    fsx.rmSync(seeds.DIR, { recursive: true, force: true });
    fsx.mkdirSync(seeds.DIR, { recursive: true });
    // The pid is read off the ledger FILENAME, so a live-but-unsignalable run is written directly.
    fsx.writeFileSync(
      path.join(seeds.DIR, "someverifier-1.json"),
      JSON.stringify([{ url: "https://app.test/api/things/REAL-42", label: "a fixture", check: "someverifier", at: new Date().toISOString() }]),
    );

    const tool = path.join(BUNDLE_ROOT, ".e2e", "tools", "sweep-seeds.js");
    let out = "";
    try {
    ...
    assert.match(out, /run still alive/, "a process that answers EPERM was reported as dead");
    assert.match(
      out,
      /still running and will be skipped/,
      "the live run's fixture was offered up as sweepable",
    );
    fsx.rmSync(seeds.DIR, { recursive: true, force: true });
  });
