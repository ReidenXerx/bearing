...
describe("bearing", () => {
  ...
  it("bundle includes docs required by gitnexus-setup.mjs", () => {
    const files = listBundleFiles();
  ...
  it("shell staleness guard denies non-git shell when stale", async () => {
    ...
    fs.chmodSync(
      path.join(tmp, ".cursor/hooks/bearing-bash-guard.mjs"),
      0o755,
    );
    const r = spawnSync(
      "bash",
  ...
  it("every file a REAL install leaves behind is either exempted or deliberately not ours", () => {
    ...
    for (const runtime of ["claude", "cursor", "zed", "codex"]) {
  ...
  it("product metadata stays in sync with the code (no third-copy drift)", async () => {
    ...
    const surfaces = {
      ...
      "bin/install.mjs": fs.readFileSync(new URL("../bin/install.mjs", import.meta.url), "utf8"),
    };
  ...
  it("a module that no active runtime supports is skipped, loudly (NS-14)", async () => {
    ...
    // which only Claude Code can do. Overstated parity, shipped by the installer itself.
    const mk = () => {
    ...
    assert.ok(
      !fs.existsSync(path.join(cursorOnly, ".bearing/skills/bearing-minions")),
      "the skill installed for a runtime that cannot use it",
    );
    const rule = fs.readFileSync(
      path.join(cursorOnly, "templates/CLAUDE.gitnexus.md"),
      "utf8",
    );
    assert.doesNotMatch(rule, /bearing-minions/, "Cursor's rule told the agent to fan out");
    ...
    for (const t of [cursorOnly, withClaude, zed]) fs.rmSync(t, { recursive: true, force: true });
  });
  ...
  it("post-install checks pass on a good install, and each one can fail", async () => {
    ...
    //    covering the TOKEN.
    for (const doc of ["CLAUDE.md", "templates/CLAUDE.gitnexus.md"]) {
  ...
  it("the persona is persisted, injected into the contract, and never overwritten", async () => {
    ...
    const personas = ["CLAUDE.md", "AGENTS.md", "templates/CLAUDE.gitnexus.md"].map(
    ...
    for (const f of ["CLAUDE.md", "AGENTS.md", "templates/CLAUDE.gitnexus.md"]) {
  ...
  it("packs every .bearing/lib module it ships", () => {
    ...
    const pack = fs.readFileSync(path.join(BUNDLE_ROOT, "scripts/pack-bearing-teaching.mjs"), "utf8");
  ...
  it("bearing-setup.sh only requires files a stealth install actually creates", () => {
    ...
    const setup = fs.readFileSync(path.join(BUNDLE_ROOT, "scripts/bearing-setup.mjs"), "utf8");
    const block = setup.match(/if is_stealth; then\s*CLAUDE_SOURCES=\(([^)]*)\)/)?.[1] ?? "";
    const required = [...block.matchAll(/"([^"]+)"/g)].map((m) => m[1]);
    assert.ok(required.length, "could not read the stealth CLAUDE_SOURCES out of bearing-setup.mjs");
  ...
  it("the teaching sync reads the recorded runtime, not a default that assumes Cursor", () => {
    ...
    const sync = fs.readFileSync(
      path.join(BUNDLE_ROOT, "scripts/sync-cursor-bearing-teaching.mjs"),
      "utf8",
    );
    assert.ok(
  ...
  it("a failing post-refresh step does not report the refresh itself as failed", () => {
    ...
    assert.ok(
      !/\n\s*run\("bash", \["scripts\/sync-cursor-bearing-teaching\.mjs"\]/.test(refresh),
  ...
  it("detects the editor you are actually running before defaulting the runtime", async () => {
    ...
    // the silent default is `both` — which is cursor+zed and covers Claude Code not at all.
    //
    ...
    fs.mkdirSync(path.join(tmp, ".cursor"), { recursive: true });
    assert.deepEqual(detectRuntimes(tmp, {}), ["cursor"]);
    fs.mkdirSync(path.join(tmp, ".zed"), { recursive: true });
    assert.deepEqual(detectRuntimes(tmp, {}).sort(), ["cursor", "zed"]);

    // Both kinds of evidence combine, deduped and stable.
    assert.deepEqual(detectRuntimes(tmp, { CLAUDECODE: "1" }).sort(), ["claude", "cursor", "zed"]);
    fs.rmSync(tmp, { recursive: true, force: true });
  });
  ...
  it("the hook-lib check only inspects files bearing installed", async () => {
    ...
    execSync("git init -q", { cwd: tmp });
    installKit(tmp, { runtime: "cursor", features: "all", runSetup: false, skipVerify: true });

    // A script the USER owns, naming a lib bearing never installs.
    fs.mkdirSync(path.join(tmp, "scripts"), { recursive: true });
    fs.writeFileSync(
      path.join(tmp, "scripts/my-own-tool.mjs"),
      "#!/usr/bin/env bash\n# copies .bearing/lib/region-infer.mjs from somewhere else\n",
    );

    const r = spawnSync("bash", ["scripts/sync-cursor-bearing-teaching.mjs"], {
      cwd: tmp,
      encoding: "utf8",
      env: { ...process.env, GITNEXUS_RUNTIME: "cursor" },
    });
    assert.equal(
      r.status,
      0,
    ...
    const broken = spawnSync("bash", ["scripts/sync-cursor-bearing-teaching.mjs"], {
      cwd: tmp,
      encoding: "utf8",
      env: { ...process.env, GITNEXUS_RUNTIME: "cursor" },
    });
  ...
  it("the Cursor hooks fail open too — same class, different runtime", () => {
    // The Claude fix was half the problem. Cursor's hooks are shell scripts under `set -euo
    // pipefail` wrapping a node heredoc, so a missing `.bearing/lib` threw inside node and `set -e`
    // turned it into a non-zero hook exit — NINE of twelve at once, including every guard.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-curfailopen-"));
    execSync("git init -q", { cwd: tmp });
    installKit(tmp, { runtime: "cursor", features: "all", runSetup: false, skipVerify: true });
    fs.rmSync(path.join(tmp, ".bearing"), { recursive: true, force: true });

    const hooks = fs.readdirSync(path.join(tmp, ".cursor/hooks")).filter((f) => f.endsWith(".mjs"));
    assert.ok(hooks.length >= 10, "no cursor hooks to check");

    const failed = [];
    for (const h of hooks) {
      const r = spawnSync("bash", [path.join(tmp, ".cursor/hooks", h)], {
        input: JSON.stringify({ tool_name: "Grep", cwd: tmp, tool_input: { pattern: "x" } }),
        encoding: "utf8",
      });
      if (r.status !== 0) failed.push(`${h} → exit ${r.status}`);
    }
    assert.deepEqual(
      failed,
      [],
      `cursor hooks that fail when .bearing is gone:\n  ${failed.join("\n  ")}`,
    );
    fs.rmSync(tmp, { recursive: true, force: true });
  });
  ...
  it("every runtime the fallback message NAMES gets a command that installs it", () => {
    ...
    // directory — so a codex user with no TTY lands on the `both` fallback, which is cursor+zed.
    ...
    // `(cursor+zed)` there are what you are GETTING, not a case needing a command, and counting
  ...
  it("every installable module has a row in the runtime parity table", () => {
    ...
    const readme = fs.readFileSync(new URL("../README.md", import.meta.url), "utf8");
    const table = readme.slice(readme.indexOf("| | Claude Code | Cursor | Zed | Codex |"));
    ...
    // And the table must not promise a runtime the module does not declare.
    const header = rows.split("\n")[0].split("|").map((c) => c.trim()).filter(Boolean);
    assert.deepEqual(header, ["Claude Code", "Cursor", "Zed", "Codex"], "parity table columns moved");
  });
  ...
  it("the shell allowlist names the scripts that actually exist", () => {
    // It kept naming `scripts/gitnexus-agent.mjs` after the rename to `bearing-*`. Nothing broke
    // loudly — every branch of that hook ends in `allow` — so the only symptom was the agent losing
    // its pre-approval and starting to ask the user for permission to run bearing's own
    // maintenance, which is the one thing the hook exists to prevent. Legacy names stay as
    // aliases (NS-15); what must not happen is the CURRENT name being absent.
    const sh = fs.readFileSync(
      path.join(BUNDLE_ROOT, ".cursor/hooks/bearing-shell-allowlist.mjs"),
      "utf8",
    );
    const patterns = [...sh.matchAll(/\/\\b(.+?)\/\.test\(command\)/g)].map((m) =>
      new RegExp(`\\b${m[1]}`),
    );
    assert.ok(patterns.length >= 4, "could not parse the allowlist patterns");
    for (const cmd of [
      "node scripts/bearing-agent.mjs fallback \"why\"",
      "bash scripts/bearing-setup.mjs",
      "npm run bearing:agent-refresh",
    ]) {
      assert.ok(
        patterns.some((re) => re.test(cmd)),
        `the allowlist does not pre-approve "${cmd}", so the agent will stop and ask`,
      );
    }
  });
