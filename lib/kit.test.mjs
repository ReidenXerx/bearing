...
describe("gitnexus-agent-kit", () => {
  ...
  it("release and skills docs are present", () => {
    ...
    assert.equal(pkg.version, "1.0.0"); // first public release
  });
  ...
  it("north-stars: tracked path, digest parsing, anchor counter", async () => {
    ...
    // The filename must NOT be dot-prefixed: the managed .gitignore covers `.gnkit/.gitnexus-*`,
  ...
  it("drift counts DELETED source files, and clears once the index is rebuilt", () => {
    ...
    const libSrc = new URL("../bundle/.gnkit/lib/", import.meta.url).pathname;
    const build = (deleteBeforeIndexing) => {
      ...
      sh("git add -A");
      sh("git commit -qm init");
      fs.mkdirSync(path.join(t, ".gnkit/lib"), { recursive: true });
      for (const f of fs.readdirSync(libSrc)) {
        fs.copyFileSync(path.join(libSrc, f), path.join(t, ".gnkit/lib", f));
      }
      ...
      const out = execSync("node .gnkit/lib/check-staleness.mjs", { cwd: t, encoding: "utf8" });
  ...
  it("feature modules: a disabled feature never ships, core always does", async () => {
    ...
    assert.equal(
      shouldCopyBundleFile(".claude/hooks/gitnexus-grep-guard.mjs", "claude", intel),
      false,
      "enforcement gate must not ship without GitNexus",
    );
    assert.equal(
      shouldCopyBundleFile(".claude/hooks/gitnexus-northstar-anchor.mjs", "claude", intel),
      true,
    );
    // Core ships regardless of which features are chosen.
    for (const f of [parseFeatures("northstars"), parseFeatures("all")]) {
      assert.equal(shouldCopyBundleFile(".gnkit/lib/session-primer.mjs", "claude", f), true);
      assert.equal(shouldCopyBundleFile(".claude/hooks/gitnexus-session.mjs", "claude", f), true);
    }
    // taskcore owns its lib; it must not ship when taskcore is off.
    assert.equal(
      shouldCopyBundleFile(".gnkit/lib/context-pressure.mjs", "claude", parseFeatures("northstars")),
      false,
    );
    assert.equal(shouldCopyBundleFile(".gnkit/lib/context-pressure.mjs", "claude", intel), true);

    // Back-compat: omitting features keeps the old behaviour (everything for the runtime).
    assert.equal(shouldCopyBundleFile(".claude/hooks/gitnexus-grep-guard.mjs", "claude"), true);
    ...
    assert.equal(featureOf("skills/gitnexus-northstars/SKILL.md"), "northstars");
    assert.equal(featureOf("skills/gitnexus-microscope/SKILL.md"), "microscope");
    assert.equal(featureOf("skills/gitnexus-debugging/SKILL.md"), "gitnexus");
    assert.equal(featureOf(".gnkit/lib/hook-helpers.mjs"), null, "core");
  });
  ...
  it("intel-only install: no GitNexus files, and every installed hook still RUNS", () => {
    ...
    assert.deepEqual(
      fs.readdirSync(path.join(tmp, ".gnkit/skills")).sort(),
      ["gitnexus-microscope", "gitnexus-northstars", "gitnexus-taskcore"],
