...
describe("gitnexus-agent-kit", () => {
  ...
  it("drift counts DELETED source files, and clears once the index is rebuilt", () => {
    ...
    // longer exist. There is no file left to stat, so the parent directory's mtime stands in as the
    // deletion timestamp — which preserves the "only counts if newer than indexedAt" discipline, so
    // a pending deletion does not block every graph query until it happens to be committed.
    const mk = () => {
    ...
    const stamp = (t) =>
      fs.writeFileSync(
        ...
        JSON.stringify({ indexedAt: new Date().toISOString(), stats: { nodes: 10 } }),
      );

    // Deleted AFTER indexing → counts as drift.
    const a = mk();
    stamp(a);
    fs.rmSync(path.join(a, "src/a.js"));
    const goneAfter = fs.statSync(path.join(a, "src")).mtimeMs > Date.parse(
      JSON.parse(fs.readFileSync(path.join(a, ".gitnexus/meta.json"), "utf8")).indexedAt,
    );
    assert.ok(goneAfter, "parent dir mtime advances past indexedAt when a file is removed");

    // Deleted BEFORE indexing (i.e. the index was rebuilt after) → must NOT count, or a pending
    // deletion would gate every query forever.
    const b = mk();
    fs.rmSync(path.join(b, "src/a.js"));
    stamp(b);
    const goneBefore = fs.statSync(path.join(b, "src")).mtimeMs > Date.parse(
      JSON.parse(fs.readFileSync(path.join(b, ".gitnexus/meta.json"), "utf8")).indexedAt,
    );
    assert.ok(!goneBefore, "a rebuilt index overtakes the deletion — signal clears");
