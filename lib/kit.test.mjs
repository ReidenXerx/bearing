...
describe("bearing", () => {
  ...
  it("the .prettierignore block covers every formattable file an install actually ships", async () => {
    // The pattern list is written by hand, because a .prettierignore nobody can read is worse than
    // one that is slightly redundant. This is the GP-11 half of that bargain: the CHECK is computed,
    // so a new bundle file that no pattern matches fails here instead of being quietly reformatted
    // and overwritten forever. It caught scripts/lib/setup-ui.mjs on the first run.
    const { prettierIgnoreLines } = await import(new URL("./prettier.mjs", import.meta.url).href);
    const { parseFeatures } = await import(new URL("./features.mjs", import.meta.url).href);
    const match = (pat, rel) => {
    ...
    for (const [runtime, features] of [
      ["all", null],
      ["claude", parseFeatures("northstars,taskcore")],
      ["cursor,zed", parseFeatures("gitnexus")],
    ]) {
      const shipped = listBundleFiles().filter(
        (r) => shouldCopyBundleFile(r, runtime, features) && FORMATTABLE.test(r),
      );
      assert.ok(shipped.length, `${runtime}: nothing shipped — the probe is wrong, not the bundle`);
      const pats = prettierIgnoreLines(runtime, features);
      const missed = shipped.filter((r) => !pats.some((pat) => match(pat, r)));
      assert.deepEqual(
        missed,
        [],
        `${runtime}: shipped but not exempted — Prettier will reformat these and the next update will overwrite them back`,
      );
    }
  });
