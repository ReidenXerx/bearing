...
describe("bearing", () => {
  ...
  it("installKit claude runtime wires MCP, hooks, CLAUDE.md, skills", () => {
    ...
    // Status line: the script ships and settings.json points at it.
    assert.ok(
      fs.existsSync(path.join(tmp, ".claude/statusline.sh")),
      "status line script ships for claude",
    );
    assert.match(
      settings.statusLine?.command ?? "",
      /statusline\.sh/,
      "statusLine wired to the shipped script",
    );
    // Run it through `bash`, never as a bare path: chmodScripts() does not cover
    // .claude/, so the file arrives without +x and a direct exec would EACCES.
    assert.match(
      settings.statusLine?.command ?? "",
      /^bash /,
      "status line is invoked through bash, not exec'd directly",
    );
    const m = readManifest(tmp);
  ...
  it("the shipped status line renders a status payload without erroring", () => {
    ...
    const script = path.join(BUNDLE_ROOT, ".claude/statusline.sh");
  ...
  it("a status line the user wrote is never clobbered, on install or uninstall", () => {
    // Overwriting someone's status line is destructive AND invisible: they find out by
    // noticing the bottom of their screen changed. Ours is the only one we may touch.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-statusline-"));
    execSync("git init -q", { cwd: tmp });
    execSync("git config user.email t@t.com && git config user.name t", {
      cwd: tmp,
      shell: true,
    ...
    const mine = { type: "command", command: "~/bin/my-own-statusline.sh" };
    fs.writeFileSync(
      path.join(tmp, ".claude/settings.json"),
      JSON.stringify({ statusLine: mine }, null, 2),
    );
    installKit(tmp, {
      runtime: "claude",
      quick: true,
    ...
    assert.deepEqual(read().statusLine, mine, "install leaves their status line alone");
    uninstallKit(tmp);
    const after = fs.existsSync(path.join(tmp, ".claude/settings.json"))
      ? read()
      : {};
    assert.deepEqual(after.statusLine, mine, "uninstall leaves it alone too");
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("bearing's own status line is removed on uninstall", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "gn-statusline-own-"));
    execSync("git init -q", { cwd: tmp });
    execSync("git config user.email t@t.com && git config user.name t", {
      cwd: tmp,
      shell: true,
    ...
    uninstallKit(tmp);
    // The file is deleted outright when ours was the only thing in it.
    const after = fs.existsSync(settingsPath)
      ? JSON.parse(fs.readFileSync(settingsPath, "utf8"))
      : {};
    assert.equal(after.statusLine, undefined, "ours does not outlive the uninstall");
    fs.rmSync(tmp, { recursive: true, force: true });
  });
